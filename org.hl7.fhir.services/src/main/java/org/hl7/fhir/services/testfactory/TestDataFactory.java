...
import org.hl7.fhir.services.testfactory.dataprovider.TableDataProvider;
import org.hl7.fhir.services.testfactory.dataprovider.ValueSetDataProvider;
...
public class TestDataFactory {
  ...
  private void executeProfile() throws IOException {
    try {
      ...
      StructureDefinition profile = context.fetchResource(StructureDefinition.class, purl, VersionResolutionRules.defaultRule());
      if (profile == null) {
  ...
  private TableDataProvider loadTable(String path, TableDataProvider.SheetInfo sheetInfo) throws IOException {
  ...
  public TableDataProvider loadTableProvider(String path, TableDataProvider.SheetInfo sheetInfo, Locale locale) {
    ...
    if (Utilities.isAbsoluteUrl(path)) {
      ValueSet vs = context.findTxResource(ValueSet.class, path, VersionResolutionRules.defaultRule());
      if (vs == null) {
