...
public class StructureDefinitionSpreadsheetGenerator extends CanonicalSpreadsheetGenerator {
  ...
  public StructureDefinitionSpreadsheetGenerator renderStructureDefinition(StructureDefinition sd, boolean forMultiple) throws Exception {
    ...
    for (ElementDefinition child : sd.getSnapshot().getElementList()) {
  ...
  private void addStructureDefinitionMetadata(Sheet sheet, StructureDefinition sd) {
    for (Coding k : sd.getKeywordList()) {
    ...
    for (StructureDefinitionContextComponent k : sd.getContextList()) {
      addMetadataRow(sheet, "Context", k.getTypeElement().asStringValue()+":"+k.getExpression());
    }
    for (StringType k : sd.getContextInvariantList()) {
  ...
  public void processElement(Sheet sheet, StructureDefinition sd, ElementDefinition ed, boolean forMultiple) throws Exception {
    ...
    addCell(row, i++, itemList(ed.getAliasList()));
    ...
    addCell(row, i++, itemList(ed.getTypeList()));
    ...
    addCell(row, i++, ed.hasExample() ? renderType(ed.getExampleList().get(0).getValue()) : ""); // todo...?
    ...
    addCell(row, i++, itemList(ed.getCodeList()));
    if (ed.hasSlicing()) {
      addCell(row, i++, itemList(ed.getSlicing().getDiscriminatorList()));
    ...
    addCell(row, i++, itemList(ed.getConditionList()));
    addCell(row, i++, itemList(ed.getConstraintList()));
    if (!forMultiple) {
      for (StructureDefinitionMappingComponent mapKey : sd.getMappingList()) {
        String mapString = "";
    ...
    for (int i =0; i< l.size(); i++) {
      ...
      if (o instanceof StringType) {
        ...
        if (t.hasTargetProfile()) 
          val = val+ "(" + canonicalList(t.getTargetProfileList()) + ")";
        if (t.hasProfile())
          val = val + " {" + canonicalList(t.getProfileList()) + "}";
        if (t.hasAggregation()) 
          val = val + " <<" + aggList(t.getAggregationList()) + ">>";
  ...
  private String aggList(List<org.hl7.fhir.model.core.Enumeration<AggregationMode>> list) {
    CommaSeparatedStringBuilder b = new CommaSeparatedStringBuilder();
    for (org.hl7.fhir.model.core.Enumeration<AggregationMode> c : list)
  ...
  public void configureSheet(Sheet sheet, StructureDefinition sd) throws IOException {
    ...
    if (hideMustSupportFalse) {
      ...
      sheet.setAutoFilter(new CellRangeAddress(0,sheet.getLastRowNum(), 0, titles.length+(sd == null ? 0 : sd.getMappingList().size() - 1)));
