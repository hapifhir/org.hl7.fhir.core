...
public class ResourceParser extends ParserBase {
  ...
  public Element parse(Resource resource) {
    ...
    Property p = new Property(context, sd.getSnapshot().getElementList().get(0), sd, getProfileUtilities(), getContextUtilities());
  ...
  private void parseChildren(String path, Base src, Element dst) {
    ...
    for (org.hl7.fhir.model.Property c : src.getChildren()) {
      if (c.hasValues()) {
...
//            }
          ...
          n.setPath(npath);
          if (p.isResource()) {
            StructureDefinition sd = context.fetchResource(StructureDefinition.class, ProfileUtilities.sdNs(v.fhirType(), null), VersionResolutionRules.defaultRule());
            if (sd == null)
            ...
            n.updateProperty(new Property(context, sd.getSnapshot().getElementList().get(0), sd, getProfileUtilities(), getContextUtilities()), SpecialElement.fromProperty(n.getProperty()), p);
