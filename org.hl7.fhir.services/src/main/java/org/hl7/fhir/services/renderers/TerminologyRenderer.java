...
public abstract class TerminologyRenderer extends ResourceRenderer {
  ...
  protected List<TargetElementComponentWrapper> findMappingsForCode(String code, ConceptMap map) {
    ...
    for (ConceptMapGroupComponent g : map.getGroupList()) {
      for (SourceElementComponent c : g.getElementList()) {
        if (c.getCode().equals(code))
          for (TargetElementComponent cc : c.getTargetList())
  ...
  protected void addCsRef(ConceptSetComponent inc, XhtmlNode li, CodeSystem cs) {
    ...
    VersionResolutionRules resolutionMethod = ExtensionUtilities.getVersionResolutionRules(inc);
  ...
  protected String getDisplayForProperty(String uri) {
    ...
    CodeSystem cs = getContext().getWorker().fetchCodeSystem(uri, VersionResolutionRules.defaultRule());
    if (cs == null) {
      return null;
    }
  ...
  protected void AddVsRef(String value, Base ctxt, XhtmlNode li, Resource source) {
    ...
    VersionResolutionRules rules = ExtensionUtilities.getVersionResolutionRulesBase(ctxt);
