...
public class ConceptMapRenderer extends TerminologyRenderer {
  ...
  public void render(RenderingStatus status, ResourceWrapper res, XhtmlNode x, ConceptMap cm, boolean header) throws FHIRFormatError, DefinitionException, IOException {
    ...
    for (ConceptMapGroupComponent grp : cm.getGroupList()) {
      ...
      for (SourceElementComponent ccl : grp.getElementList()) {
        isSimple = isSimple && (ccl.getNoMap() || (ccl.getTargetList().size() == 1 && ccl.getTargetList().get(0).getDependsOnList().isEmpty() && ccl.getTargetList().get(0).getProductList().isEmpty()));
        if (ccl.hasExtension(ExtensionDefinitions.EXT_CM_NOMAP_COMMENT)) {
          hasComment = true;
        }
        for (TargetElementComponent ccm : ccl.getTargetList()) {
          hasComment = hasComment || !Utilities.noString(ccm.getComment());
          for (MappingPropertyComponent pp : ccm.getPropertyList()) {
            if (!props.containsKey(pp.getCode()))
              props.put(pp.getCode(), new HashSet<String>());            
          }
          for (OtherElementComponent d : ccm.getDependsOnList()) {
            if (!sources.containsKey(d.getAttribute()))
              sources.put(d.getAttribute(), new HashSet<String>());
          }
          for (OtherElementComponent d : ccm.getProductList()) {
  ...
  private void renderCodeSystemMap(RenderingStatus status, ResourceWrapper res, XhtmlNode x, int gc, String eqpath,
    ...
    if (isSimple) {
      ...
      for (int si = 0; si < grp.getElementList().size(); si++) {
        SourceElementComponent ccl = grp.getElementList().get(si);
        for (int ti = 0; ti < ccl.getTargetList().size(); ti++) {
          TargetElementComponent ccm = ccl.getTargetList().get(ti);
          if (ccm.hasRelationship()) {
      ...
      for (int si = 0; si < grp.getElementList().size(); si++) {
        SourceElementComponent ccl = grp.getElementList().get(si);
        boolean slast = si == grp.getElementList().size()-1;
        boolean first = true;
        if (ccl.hasNoMap() && ccl.getNoMap()) {
          ...
          for (int ti = 0; ti < ccl.getTargetList().size(); ti++) {
            TargetElementComponent ccm = ccl.getTargetList().get(ti);
            boolean last = ti == ccl.getTargetList().size()-1;
            tr = tbl.tr();
            td = tr.td().style("border-right-width: 0px");
            if (!first && !last)
            ...
            for (String s : sources.keySet()) {
              if (s != null && !s.equals("code")) {
                td = tr.td();
                td.addText(getValue(ccm.getDependsOnList(), s, sources.get(s).size() != 1));
                display = getDisplay(ccm.getDependsOnList(), s);
                if (display != null)
                  td.tx(" ("+display+")");
              }
            }
            ...
            for (String s : targets.keySet()) {
              if (s != null && !s.equals("code")) {
                td = tr.td();
                td.addText(getValue(ccm.getProductList(), s, targets.get(s).size() != 1));
                display = getDisplay(ccm.getProductList(), s);
                if (display != null)
                  td.tx(" ("+display+")");
              }
            }
  ...
  private void renderPropValues(Map<String, HashSet<String>> props, TargetElementComponent ccm, XhtmlNode tr) {
    ...
    if (!props.isEmpty()) {
      for (String s : props.keySet()) {
        if (s != null) {
          td = tr.td();
          td.addText(getValue(ccm.getPropertyList(), s));
        }
      }
    }
  }
  ...
  public void renderCSDetailsLink(XhtmlNode tr, String url, boolean span2) {
    CodeSystem cs;
    XhtmlNode td;
    cs = getContext().getWorker().fetchCodeSystem(url, VersionResolutionRules.defaultRule());
    td = tr.td();
    if (span2) {
      td.colspan("2");
    }
...
//      throw new Error("not done yet");
  ...
  private static void collateRows(List<MultipleMappingRow> rowSets) {
    List<MultipleMappingRow> toDelete = new ArrayList<MultipleMappingRow>();
