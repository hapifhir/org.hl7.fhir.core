  ...
  private void renderBinding(XhtmlNode x, TableElementConstraint c, String phrase) {
    ...
    if (vs == null) {
      ...
      try {      
        ...
        } else if (exp.getValueset().getExpansion().getContainsList().size() == 1000) {
          x.tx(" (>1000 codes)");                            
        } else if (exp.getValueset().getExpansion().getContainsList().size() > 6) {
          x.tx(" ("+exp.getValueset().getExpansion().getContainsList().size()+" codes)");
        } else {
          x.tx(".  Codes:");
          XhtmlNode ul = x.ul();
  ...
  private void renderChoiceConstraint(XhtmlNode x, TableElementConstraint c) {
    ...
    for (TypeRefComponent tr : c.types) {
      if (tr.hasProfile()) {
        ...
        li.tx(" pointing to ");
        renderTypeList(x, tr.getTargetProfileList());
