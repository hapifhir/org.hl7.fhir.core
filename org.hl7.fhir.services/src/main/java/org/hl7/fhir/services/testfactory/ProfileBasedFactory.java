...
import org.hl7.fhir.services.testfactory.TestDataFactory.DataTable;
import org.hl7.fhir.services.testfactory.dataprovider.BaseDataTableProvider;
...
public class ProfileBasedFactory {
      ...
      if (b.isPrimitive()) {        
        ...
        if (values == null || val != null || pe.min() > 0) {
          ...
          if (val == null && pe.valueSet() != null) {
            ValueSet.ValueSetExpansionContainsComponent cc = doExpansion(ls, pe.valueSet());
            if (cc != null) {
              val = cc.getCode();
            }
          }
        ...
        if (build) {
          ...
          if (values == null) {
            if ("Reference".equals(b.fhirType()) && values == null) {
              ...
              for (ElementDefinition.TypeRefComponent tr : pe.definition().getTypeList()) {
                if (tr.getWorkingCode().equals("Reference")) {
                  targets.addAll(tr.getTargetProfileList());
                }
              }
  ...
  private ValueSet.ValueSetExpansionContainsComponent doExpansion(LogSet ls, ValueSet vs) {
    ...
    if (vse.isOk()) {
      ls.others.add("ValueSet "+vs.getVersionedUrl()+" "+ ValueSetUtilities.countExpansion(vse.getValueset().getExpansion().getContainsList())+" concepts");
      if (testing) {
        for (ValueSet.ValueSetExpansionContainsComponent cc : vse.getValueset().getExpansion().getContainsList()) {
          ls.others.add(cc.getSystem()+"#"+cc.getCode()+" : \""+cc.getDisplay()+"\" ("+cc.hasContains()+")");   
        }
      }
      return pickRandomConcept(vse.getValueset().getExpansion().getContainsList());
    } else {
    ...
    while (res == null && list.size() > 0) {
      ...
      if (list.get(r).getAbstract()) {
        if (list.get(r).hasContains()) {
          res = pickRandomConcept(list.get(0).getContainsList());
        }
      } else {
        res = list.get(r);
      }
  ...
  private void populateElementFromDataType(Element element, Base source, PEDefinition defn) {
    for (Property prop : source.getChildren()) {
