...
//    }
  ...
  private void generateExpansion(RenderingStatus status, ResourceWrapper res, XhtmlNode x, ValueSet vs, boolean header, List<UsedConceptMap> maps) throws FHIRFormatError, DefinitionException, IOException {
    ...
    if (ExtensionUtilities.hasExtension(vs.getExpansion(), ExtensionDefinitions.EXT_EXP_TOOCOSTLY)) {
      String msg = null;
      if (vs.getExpansion().getContainsList().isEmpty()) {
    ...
    boolean doInactive = checkDoInactive(vs.getExpansion().getContainsList());    
    boolean doDefinition = checkDoDefinition(vs, vs.getExpansion().getContainsList());
    boolean doVersion = checkDoVersion(vs.getExpansion().getContainsList());
    ...
    if (displang == null) {
      displang = findParamValue(vs.getExpansion().getParameterList(), "displayLanguage");
    }
    if (displang == null) {
  ...
  private void scanForProperties(ValueSetExpansionComponent exp, List<String> langs, Map<String, String> properties) {
    properties.clear();
    for (ValueSetExpansionPropertyComponent pp : exp.getPropertyList()) {
      if (pp.hasCode() && pp.hasUri() && anyActualproperties(exp.getContainsList(), pp.getCode())) {
  ...
  private boolean anyActualproperties(List<ValueSetExpansionContainsComponent> contains, String pp) {
    for (ValueSetExpansionContainsComponent c : contains) {
      for (ConceptPropertyComponent cp : c.getPropertyList()) {
        if (pp.equals(cp.getCode())) {
          return true;
        }
      }
      if (anyActualproperties(c.getContainsList(), pp)) {
        return true;
      }
    }
    return false;
  ...
  private boolean generateContentModeNotice(XhtmlNode x, ValueSetExpansionComponent expansion, String mode, String text, Resource vs) {
    ...
    for (ValueSetExpansionParameterComponent p : expansion.getParameterList()) {
    ...
    if (vs.hasExpansion())
      ...
      if (vs.hasCompose()) {
        ...
        for (ConceptSetComponent inc : vs.getCompose().getIncludeList()) {
          ...
          if (!inc.hasConcept())
            return null;
          count = count + inc.getConceptList().size();
        }
      }
    }
    return count;
  ...
  private void expRef(XhtmlNode x, String u, String v, Resource source) {
    ...
    if (u.equals("http://snomed.info/sct")) {
      ...
      CanonicalResource cr = (CanonicalResource) getContext().getWorker().fetchResource(Resource.class, u, VersionResolutionRules.defaultRule(), null, source);
      if (cr != null) {
        if (cr.hasWebPath()) {
      ...
      CanonicalResource cr = (CanonicalResource) getContext().getWorker().fetchResource(Resource.class, u, VersionResolutionRules.defaultRule(), v, source);
      if (cr != null) {
        if (cr.hasWebPath()) {
  ...
  public void addDesignationsToRow(ValueSetExpansionContainsComponent c, Map<String, String> designations, XhtmlNode tr) {
    for (String url : designations.keySet()) {
      String d = null;
      if (d == null) {
        for (ConceptReferenceDesignationComponent dd : c.getDesignationList()) {
  ...
  private boolean checkDoDefinition(ValueSet source, List<ValueSetExpansionContainsComponent> contains) {
    for (ValueSetExpansionContainsComponent c : contains) {
      ...
      if (checkDoDefinition(source, c.getContainsList()))
        return true;
    }
    return false;
  }
  ...
  private CodeSystem getFetchedCodeSystem(String system, String version, ValueSet source) {
    ...
    CodeSystem cs = getContext().getWorker().fetchSupplementedCodeSystem(system, VersionResolutionRules.defaultRule(), version, null, source);
  ...
  private boolean checkDoInactive(List<ValueSetExpansionContainsComponent> contains) {
    for (ValueSetExpansionContainsComponent c : contains) {
      ...
      if (checkDoInactive(c.getContainsList()))
        return true;
    }
    return false;
  }
  ...
  private boolean checkDoVersion(List<ValueSetExpansionContainsComponent> contains) {
    for (ValueSetExpansionContainsComponent c : contains) {
      ...
      if (checkDoVersion(c.getContainsList()))
        return true;
    }
    return false;
  }
  ...
  private boolean allFromOneSystem(ValueSet vs) {
    if (vs.getExpansion().getContainsList().isEmpty())
      return false;
    String system = vs.getExpansion().getContainsList().get(0).getSystem();
    for (ValueSetExpansionContainsComponent cc : vs.getExpansion().getContainsList()) {
  ...
  private void scanForDesignations(ValueSet vs, ValueSetExpansionContainsComponent c, List<String> langs, Map<String, String> designations) {
    ...
    if (context.getDesignationMode() != DesignationMode.NONE) {
      for (ConceptReferenceDesignationComponent d : c.getDesignationList()) {
  ...
  private void addExpansionRowToTable(XhtmlNode t, ValueSet vs, ValueSetExpansionContainsComponent c, int i, boolean doLevel, boolean doDefinition, boolean doInactive, boolean doVersion, List<UsedConceptMap> maps, List<String> langs, Map<String, String> designations, boolean doDesignations, Map<String, String> properties, ResourceWrapper res) throws FHIRFormatError, DefinitionException, IOException {
    ...
    if (context.forPublisher()) {
      ...
      String json = makeJson(c, getVersionForSystem(vs.getExpansion().getParameterList(), c.getSystem()));
      ...
      String xml = makeXml(c, getVersionForSystem(vs.getExpansion().getParameterList(), c.getSystem()));
  ...
  private boolean checkSystemMatches(String system, ValueSetExpansionContainsComponent cc) {
    if (!system.equals(cc.getSystem()))
      return false;
    for (ValueSetExpansionContainsComponent cc1 : cc.getContainsList()) {
  ...
  private void addCodeToTable(ValueSet vs, boolean isAbstract, String system, String version, String code, String display, XhtmlNode td) {
    ...
    if (e == null || (e.getContent() != org.hl7.fhir.model.core.Enumerations.CodeSystemContentMode.COMPLETE && e.getContent() != org.hl7.fhir.model.core.Enumerations.CodeSystemContentMode.FRAGMENT)) {
      if (isAbstract)
...
//    a.addText(code);
  ...
  private void generateComposition(RenderingStatus status, ResourceWrapper res, XhtmlNode x, ValueSet vs, boolean header, List<UsedConceptMap> maps) throws FHIRException, IOException {
    ...
    int index = 0;
    if (vs.getCompose().getIncludeList().size() == 1 && vs.getCompose().getExcludeList().size() == 0 && !VersionComparisonAnnotation.hasDeleted(vs.getCompose(), "include", "exclude")) {
      genInclude(status, x.ul(), vs.getCompose().getIncludeList().get(0), "Include", langs, doDesignations, maps, designations, index, vs);
    } else {
      XhtmlNode p = x.para();
    ...
    if (!doDesignations && langs.size() + designations.size() > 0) {
      ...
      for (ConceptSetComponent c : vs.getCompose().getIncludeList()) {
        for (ConceptReferenceComponent cc : c.getConceptList()) {
          addDesignationRow(cc, t, langs, designations);
        }
      }
    }
  ...
  private void genInclude(RenderingStatus status, XhtmlNode ul, ConceptSetComponent inc, String type, List<String> langs, boolean doDesignations, List<UsedConceptMap> maps, Map<String, String> designations, int index, ValueSet vsRes) throws FHIRException, IOException {
    ...
    if (inc.hasSystem()) {
      ...
      if (inc.getConceptList().size() == 0 && inc.getFilterList().size() == 0) {
        li.addText(type+" "+ context.formatPhrase(RenderingI18nContext.VALUE_SET_ALL_CODES_DEF) + " ");
        addCsRef(inc, li, e);
      } else {
        if (inc.getConceptList().size() > 0) {
        ...
        if (inc.getFilterList().size() > 0) {
          ...
          for (int i = 0; i < inc.getFilterList().size(); i++) {
            ConceptSetFilterComponent f = inc.getFilterList().get(i);
            if (i > 0) {
              if (i == inc.getFilterList().size()-1) {
      ...
      if (inc.hasValueSet()) {
        ...
        boolean first = true;
        for (UriType vs : inc.getValueSetList()) {
          if (first)
            first = false;
          else
            li.tx(", ");
      ...
      li.tx(context.formatMessagePlural(inc.getValueSetList().size(), RenderingContext.VALUE_SET_IMPORT)+" ");
      if (inc.getValueSetList().size() <= 2) {
        int i = 0;  
        for (UriType vs : inc.getValueSetList()) {
          if (i > 0) {
            if ( i  < inc.getValueSetList().size() - 1) {
              li.tx(", ");
            } else {
              li.tx(" and ");              
            }
  ...
  private Map<String, ConceptDefinitionComponent> getConceptsForCodes(CodeSystem e, ConceptSetComponent inc, ValueSet source, int index) {
    ...
    for (ConceptReferenceComponent cc : inc.getConceptList()) {
      ...
      if (e != null && code != null) {
        v = getConceptForCode(e.getConceptList(), code);
      }
      if (v == null && vse != null) {
        v = getConceptForCodeFromExpansion(vse.getContainsList(), code);
      }
      if (v != null) {
        results.put(code, v);
      } else {
  ...
  private ConceptDefinitionComponent getConceptForCode(List<ConceptDefinitionComponent> list, String code) {
    ...
    if (code.equals(c.getCode()))
      return c;
      ConceptDefinitionComponent v = getConceptForCode(c.getConceptList(), code);
      if (v != null)
        return v;
    }
    return null;
  ...
  private ConceptDefinitionComponent getConceptForCodeFromExpansion(List<ValueSetExpansionContainsComponent> list, String code) {
    for (ValueSetExpansionContainsComponent c : list) {
      ...
      ConceptDefinitionComponent v = getConceptForCodeFromExpansion(c.getContainsList(), code);
      if (v != null)
        return v;
    }
    return null;
  ...
  private boolean inConcept(String code, ConceptDefinitionComponent c) {
    if (c.hasCodeElement() && c.getCode().equals(code))
      return true;
    for (ConceptDefinitionComponent g : c.getConceptList()) {
      if (inConcept(code, g))
        return true;
    }
    return false;
