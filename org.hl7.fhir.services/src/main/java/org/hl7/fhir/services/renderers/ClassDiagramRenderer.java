...
public class ClassDiagramRenderer {
  ...
  public ClassDiagramRenderer(String sourceFolder, String destFolder, String diagramId, String prefix, RenderingContext rc, String lang) throws IOException {
    ...
    layout = new HashMap<String, PointSpec>();
    linkLayouts = new HashMap<String, LinkInfo>();
    if (diagramId == null) {
  ...
  private Point determineMetrics(List<String> classNames) throws Exception {
    ...
    if (classNames != null) {
      for (String cn : classNames) {
        StructureDefinition sd = context.fetchResource(StructureDefinition.class, cn, VersionResolutionRules.defaultRule());
        if (sd == null) {
  ...
  private Point determineMetrics(StructureDefinition sd, ElementDefinition ed, ClassItem source, String path, String name, StructureDefinition base, ClassItemMode mode) throws FHIRException, IOException {
    ...
    if (n == null) {
      if (!path.contains(".")) {
        ...
        n = b.toString();
      } else if (constraintMode && ed.getTypeList().size() == 1) {
...
//    }
  ...
  private boolean countsAsAttribute(StructureDefinition sd, ElementDefinition ed) {
    if (ed.hasContentReference()) {
      ...
      return true;
    } else if (ed.getTypeList().isEmpty()) {
  ...
  private int addAttribute(XhtmlNode g, double left, double top, StructureDefinition sd, ElementDefinition e, String path, LineStatus ls, double height, double width) throws FHIRException, IOException  {
    ...
    if (e.hasContentReference()) {
      ...
      encodeType(div, ls, getTypeCodeForElement(e.getTypeList()));
    }
...
//        flag(div, ls, e.getIsSummary(), "\u03A3", "white", "black", rc.formatPhrase(RenderingContext.STRUC_DEF_ELE_INCLUDED), null);
        ...
        flag(div, ls, StructureDefinitionRenderer.hasNonBaseConstraints(e.getConstraintList()) || StructureDefinitionRenderer.hasNonBaseConditions(e.getConditionList()), "C", "black", "#7779e6", rc.formatPhrase(RenderingContext.STRUC_DEF_ELE_AFFECTED), null);
        ...
        if (e.hasBinding()) {
          ValueSet vs = context.fetchResource(ValueSet.class, path, VersionResolutionRules.defaultRule());
        ...
        boolean hasTS = !((e.getTypeList().isEmpty()) || (e.getTypeList().size() == 1 && !isReference(e.getTypeList().get(0).getName())));
        boolean hasBinding = e.hasBinding() && e.getBinding().getStrength() != BindingStrength.NULL;
        if (hasTS || hasBinding) {
          div.tx(ls.see(" \u00AB "));
          if (hasTS) {
            if (isReference(e.getTypeFirstRep().getWorkingCode()) && e.getTypeList().size() == 1) {
              boolean first = true;
              for (CanonicalType p : e.getTypeFirstRep().getTargetProfileList()) {
                if (first)
                  first = false;
                else 
                  div.tx(ls.see(" | "));
