...
import org.hl7.fhir.services.utilities.PublicationHacker;
...
import org.hl7.fhir.services.conformance.profile.ProfileUtilities.ElementChoiceGroup;
import org.hl7.fhir.services.conformance.profile.ProfileUtilities.ExtensionContext;
...
import org.hl7.fhir.services.renderers.mappings.ConceptMapMappingProvider;
import org.hl7.fhir.services.renderers.mappings.ModelMappingProvider;
import org.hl7.fhir.services.renderers.mappings.StructureDefinitionMappingProvider;
import org.hl7.fhir.services.renderers.utils.ElementTable;
import org.hl7.fhir.services.renderers.utils.ElementTable.*;
import org.hl7.fhir.services.renderers.utils.RenderingContext;
import org.hl7.fhir.services.renderers.utils.RenderingContext.FixedValueFormat;
  ...
  public void buildNarrative(RenderingStatus status, XhtmlNode x, ResourceWrapper r) throws FHIRFormatError, DefinitionException, IOException, FHIRException, EOperationOutcome {
    if (!r.isDirect()) {
      ...
      if (context.getStructureMode() == StructureDefinitionRendererMode.DATA_DICT) { 
        renderDict(status, sd, sd.getDifferential().getElementList(), x.table("dict", false).markGenerated(!context.forValidResource()), false, GEN_MODE_DIFF, "", r);
      } else {
...
//          log("This shouldn't happen?");
  ...
  private class InvariantWithStatus extends ItemWithStatus { 
    ...
    public void renderDetails(XhtmlNode f) { 
      ...
      f.b().attribute("title", context.formatPhrase(RenderingI18nContext.STRUC_DEF_FII)).tx(value.getKey());
      f.tx(": "); 
      if (value.hasHuman()) { 
  ...
  private ElementInStructure getElementByName(List<ElementDefinition> elements, String contentReference, StructureDefinition source) { 
    if (contentReference.contains("#")) { 
      ...
      if (!url.equals(source.getUrl())) { 
        ...
        if (source == null) { 
          throw new FHIRException(context.formatPhrase(RenderingI18nContext.STRUC_DEF_REND_UNABLE_RES, url, contentReference));
        } 
        elements = source.getSnapshot().getElementList(); 
      } 
    }  
    ...
    throw new FHIRException(context.formatPhrase(RenderingI18nContext.STRUC_DEF_CANT_FIND, contentReference, elements.toString(), source.getUrl()));
    //    return null; 
  } 
  ...
  public XhtmlNode generateGrid(String defFile, StructureDefinition profile, String imageFolder, boolean inlineGraphics, String profileBaseFileName, String corePath, String imagePath, Set<String> outputTracker, boolean diff) throws IOException, FHIRException {
    ...
    List<ElementDefinition> list = profile.getSnapshot().getElementList(); 
  ...
  private TableModel generateTableInner(RenderingStatus status, String defFile, StructureDefinition profile, boolean diff, String profileBaseFileName, boolean snapshot, String corePath, String imagePath, boolean logicalModel, boolean allInvariants, boolean mustSupport, RenderingContext rc, String anchorPrefix, ResourceWrapper res, String idSfx, HierarchicalTableGenerator gen) throws IOException {
    ...
    if (diff) {
      ...
      list = new ArrayList<>();
      list.addAll(profile.getSnapshot().getElementList());
    }
    ...
    case SUMMARY:
      ...
      break;
    default:
      throw new Error(context.formatPhrase(RenderingI18nContext.STRUC_DEF_ERROR));
    }
  ...
  private List<ModelMappingProvider> scanForMappings(StructureDefinition profile, List<ElementDefinition> list, List<Column> columns) {
    ...
    for (StructureDefinition src : structures) {
      if (includeSDForMap(src, true)) {
        for (StructureDefinitionMappingComponent map : src.getMappingList()) {
  ...
  private void scanBindings(List<Column> columns, List<ElementDefinition> list) { 
    ...
    if (cols.contains("required")) { 
      columns.add(new Column("required", context.formatPhrase(RenderingI18nContext.GENERAL_REQUIRED), context.formatPhrase(RenderingI18nContext.STRUC_DEF_CONC_SET)));
    } 
    if (cols.contains("extensible")) { 
      columns.add(new Column("extensible", context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT), context.formatPhrase(RenderingI18nContext.STRUC_DEF_APPROP_CON)));
    } 
    if (cols.contains("maximum")) { 
      columns.add(new Column("maximum", context.formatPhrase(RenderingI18nContext.STRUC_DEF_MAX), context.formatPhrase(RenderingI18nContext.STRUC_DEF_REQ_BIND)));
    } 
    if (cols.contains("minimum")) { 
      columns.add(new Column("minimum", context.formatPhrase(RenderingI18nContext.STRUC_DEF_MIN), context.formatPhrase(RenderingI18nContext.GENERAL_BIND_MIN_ALLOW)));
    } 
    if (cols.contains("candidate")) { 
      columns.add(new Column("candidate", context.formatPhrase(RenderingI18nContext.STRUC_DEF_CAND), context.formatPhrase(RenderingI18nContext.STRUC_DEF_CAND_SUB)));
    } 
    if (cols.contains("current")) { 
      columns.add(new Column("current", context.formatPhrase(RenderingI18nContext.STRUC_DEF_CURR), context.formatPhrase(RenderingI18nContext.STRUC_DEF_CURR_RULE)));
    } 
    if (cols.contains("preferred")) { 
      columns.add(new Column("preferred", context.formatPhrase(RenderingI18nContext.GENERAL_PREFERRED), context.formatPhrase(RenderingI18nContext.STRUC_DEF_PREF_CONT)));
    } 
    if (cols.contains("ui")) { 
      columns.add(new Column("ui", context.formatPhrase(RenderingI18nContext.STRUC_DEF_UI), context.formatPhrase(RenderingI18nContext.STRUC_DEF_UI_CONT)));
    } 
    if (cols.contains("starter")) { 
      columns.add(new Column("starter", context.formatPhrase(RenderingI18nContext.GENERAL_STARTER), context.formatPhrase(RenderingI18nContext.ADD_BIND_DESIG_SYS)));
    } 
    if (cols.contains("component")) { 
      columns.add(new Column("component", context.formatPhrase(RenderingI18nContext.GENERAL_COMPONENT), context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMP_DOC)));
    } 
    if (cols.contains("example")) { 
      columns.add(new Column("example", context.formatPhrase(RenderingI18nContext.GENERAL_EXAMPLE), context.formatPhrase(RenderingI18nContext.STRUC_DEF_EX_DESC)));
    } 
  } 
  ...
  public void scanBindings(Set<String> cols, List<ElementDefinition> list, ElementDefinition ed) { 
    if (ed.hasBinding()) { 
      if (ed.getBinding().hasValueSet() && ed.getBinding().hasStrength()) { 
        ...
        case EXAMPLE: 
          cols.add(context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXAM));
          break; 
        case EXTENSIBLE: 
          cols.add(context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXTENSIBLE));
          break; 
        case PREFERRED: 
          cols.add(context.formatPhrase(RenderingI18nContext.STRUC_DEF_PREFERRED));
          break; 
        case REQUIRED: 
          cols.add(context.formatPhrase(RenderingI18nContext.STRUC_DEF_REQUIRED));
          break; 
        default: 
          break; 
        } 
  ...
  private boolean scanObligations(List<Column> columns, List<ElementDefinition> list) { 
    ...
    if (cols.contains("$all")) { 
      columns.add(new Column("$all", context.formatPhrase(RenderingI18nContext.STRUC_DEF_ALL_ACTORS), context.formatPhrase(RenderingI18nContext.STRUC_DEF_OBLIG_ALL))); 
    } 
    for (String col : cols) { 
      if (!"$all".equals(col)) { 
        ActorDefinition actor = context.getWorker().fetchResource(ActorDefinition.class, col, VersionResolutionRules.defaultRule());
        if (actor == null) { 
          columns.add(new Column(col, urlTail(col), context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNDEF_ACT, col, col)+" "));           
        } else { 
          columns.add(new Column(col, actor.present(), context.formatPhrase(RenderingI18nContext.STRUC_DEF_ACT, actor.present(), actor.getWebPath())+" "));                     
        } 
      } 
    } 
    return res;
  ...
  public TableModel initCustomTable(HierarchicalTableGenerator gen, String prefix, boolean isLogical, boolean alternating, String id, boolean isActive, List<Column> columns) throws IOException { 
    ...
    for (Column col : columns) { 
      model.getTitles().add(gen.new Title(null, col.link != null ? col.link : model.getDocoRef(), (col.title), (col.hint), null, 0));       
    } 
    return model; 
  } 
  ...
  private Row genElement(RenderingStatus status, String defPath, HierarchicalTableGenerator gen, List<Row> rows, ElementDefinition element, List<ElementDefinition> all, List<StructureDefinition> profiles, boolean showMissing, String profileBaseFileName, Boolean extensions,  
    ...
    if (!(onlyInformationIsMapping(all, element) || (context.getStructureMode() == StructureDefinitionRendererMode.OBLIGATIONS && !elementOrDescendentsHaveObligations(all, element)))) { 
      ...
      if (tail(element.getPath()).equals("extension") && isExtension(element)) { 
        if (element.hasType() && element.getTypeList().get(0).hasProfile() && extensionIsComplex(element.getTypeList().get(0).getProfileList().get(0).getValue(), srcSD))
          row.setIcon("icon_extension_complex.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_COMPLEX));
        else 
          row.setIcon("icon_extension_simple.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE));
        ext = true; 
      } else if (tail(element.getPath()).equals("modifierExtension")) { 
        if (element.hasType() && element.getTypeList().get(0).hasProfile() && extensionIsComplex(element.getTypeList().get(0).getProfileList().get(0).getValue(), srcSD))
          row.setIcon("icon_modifier_extension_complex.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_COMPLEX));
        else 
          row.setIcon("icon_modifier_extension_simple.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE));
        ext = true;
      } else if (!hasDef || element.getTypeList().size() == 0) { 
        if (root && profile != null && context.getWorker().getResourceNames().contains(profile.getType())) { 
          ...
          row.setIcon("icon-object-box.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_OBJECT_BOX));
          keyRows.add(element.getId()+"."+ ExtensionUtilities.readStringExtension(element, ExtensionDefinitions.EXT_JSON_PROP_KEY));
        } else { 
          row.setIcon("icon_element.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_ELEMENT));
        } 
      } else if (hasDef && element.getTypeList().size() > 1) { 
        if (allAreReference(element.getTypeList())) { 
          row.setIcon("icon_reference.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_REFERENCE));
        } else if (element.hasExtension(ExtensionDefinitions.EXT_JSON_PRIMITIVE_CHOICE)) { 
          row.setIcon("icon_choice.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_CHOICE));
        } else { 
          row.setIcon("icon_choice.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_CHOICE));
          typesRow = row; 
        } 
      } else if (hasDef && element.getTypeList().get(0).getWorkingCode() != null && element.getTypeList().get(0).getWorkingCode().startsWith("@")) { 
        row.setIcon("icon_reuse.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_REUSE));
      } else if (hasDef && context.getContext().isPrimitiveType(element.getTypeList().get(0).getWorkingCode())) { 
        if (keyRows.contains(element.getId())) { 
          row.setIcon("icon-key.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_KEY));
        } else { 
          row.setIcon("icon_primitive.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_PRIMITIVE));
        } 
      } else if (hasDef && element.getTypeList().get(0).hasTarget()) { 
        row.setIcon("icon_reference.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_REFERENCE));
      } else if (hasDef && context.getContext().isDataType(element.getTypeList().get(0).getWorkingCode())) { 
        row.setIcon("icon_datatype.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_DATATYPE));
      } else if (hasDef && element.hasExtension(ExtensionDefinitions.EXT_JSON_PROP_KEY)) { 
        row.setIcon("icon-object-box.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_OBJECT_BOX));
        keyRows.add(element.getId()+"."+ExtensionUtilities.readStringExtension(element, ExtensionDefinitions.EXT_JSON_PROP_KEY)); 
      } else if (hasDef && Utilities.existsInList(element.getTypeList().get(0).getWorkingCode(), "Base", "Element", "BackboneElement")) { 
        row.setIcon("icon_element.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_ELEMENT));
      } else { 
        row.setIcon("icon_resource.png", context.formatPhrase(RenderingI18nContext.GENERAL_RESOURCE));
      }
      ...
      if (element.hasSlicing()) { 
        if (standardExtensionSlicing(element)) { 
          used.used = true; // doesn't matter whether we have a type, we're used if we're setting up slicing ... element.hasType() && element.getTypeList().get(0).hasProfile(); 
          showMissing = false; //? 
          slicingRow = row; 
        } else { 
          row.setIcon("icon_slice.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_SLICE)); 
          slicingRow = row; 
        ...
        row.setIcon("icon_slice_item.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_SLICE_ITEM)); 
      } 
      ...
      if (!used.used && !element.hasSlicing()) { 
        ...
        if (slicingRow != originalRow && !children.isEmpty()) { 
          ...
          hrow.setLineColor(1); 
          hrow.setIcon("icon_element.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_ELEMENT)); 
          hrow.getCells().add(gen.new Cell(null, null, sName+ (context.formatPhrase(RenderingI18nContext.STRUC_DEF_ALL_SLICES)), "", null)); 
          switch (context.getStructureMode()) { 
          case BINDINGS: 
          case OBLIGATIONS: 
          case MAPPINGS:
          ...
          case SUMMARY: 
            ...
            hrow.getCells().add(gen.new Cell()); 
            hrow.getCells().add(gen.new Cell()); 
            hrow.getCells().add(gen.new Cell(null, null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CONT_RULE), "", null)); 
            break;            
          } 
        ...
        if (typesRow != null && !children.isEmpty()) { 
          ...
          hrow.setLineColor(1); 
          hrow.setIcon("icon_element.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_ELEMENT)); 
          hrow.getCells().add(gen.new Cell(null, null, sName+ context.formatPhrase(RenderingI18nContext.STRUC_DEF_ALL_TYPES), "", null)); 
          switch (context.getStructureMode()) { 
          case BINDINGS: 
          case OBLIGATIONS: 
          case MAPPINGS:
          ...
          case SUMMARY: 
            ...
            hrow.getCells().add(gen.new Cell()); 
            hrow.getCells().add(gen.new Cell()); 
            hrow.getCells().add(gen.new Cell(null, null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CONT_TYPE), "", null)); 
          } 
          row.getSubRows().add(hrow); 
          row = hrow; 
        } 
          ...
          for (ElementDefinition child : children) { 
            ...
            if (child.hasSliceName()) { 
              ...
              if (slicer == null || !noTail(slicer.getId()).equals(child.getPath())) { 
                ...
                parent.setIcon("icon_slice.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_SLICE)); 
                parent.getCells().add(gen.new Cell(null, null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_SLICE_FOR, child.getName()), "", null)); 
                switch (context.getStructureMode()) { 
                case BINDINGS: 
                case OBLIGATIONS:
                case MAPPINGS: 
                ...
                case SUMMARY: 
                  ...
                  parent.getCells().add(gen.new Cell()); 
                  parent.getCells().add(gen.new Cell()); 
                  parent.getCells().add(gen.new Cell(null, null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CONT_RULE), "", null)); 
                  break;             
                } 
  ...
  private List<ElementDefinitionBindingAdditionalComponent> collectBindings(ElementDefinition element, String type) { 
    ...
    if (element.hasBinding()) { 
      ...
      for (ElementDefinitionBindingAdditionalComponent t : b.getAdditionalList()) { 
  ...
  public Cell genElementNameCell(HierarchicalTableGenerator gen, ElementDefinition element, String profileBaseFileName, boolean snapshot, String corePath, 
    ...
    String hint = ""; 
    hint = checkAdd(hint, element.hasSliceName() ? context.formatPhrase(RenderingI18nContext.STRUC_DEF_SLICE_PAR, element.getSliceName()) : ""); 
    ...
    if (element.hasSlicing() && slicesExist(elements, element)) { // some elements set up slicing but don't actually slice, so we don't augment the name  
      sName = context.formatPhrase(RenderingI18nContext.STRUC_DEF_SLICE_FOR, sName); 
    } 
  ...
  public List<Cell> genElementCells(RenderingStatus status, HierarchicalTableGenerator gen, ElementDefinition element, String profileBaseFileName, boolean snapshot, String corePath, 
    ...
    if (element != null && element.getIsModifier()) { 
      checkForNoChange(element.getIsModifierElement(), gc.addStyledText((context.formatPhrase(RenderingI18nContext.STRUC_DEF_MOD)), "?!", null, null, null, false)); 
    } 
    if (element != null) { 
    ...
    if (element != null && (hasNonBaseConstraints(element.getConstraintList()) || hasNonBaseConditions(element.getConditionList()))) { 
    ...
    if (element != null && element.hasExtension(ExtensionDefinitions.EXT_STANDARDS_STATUS)) { 
      ...
      gc.addStyledText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_STAND_STATUS) +ss.toDisplay(), ss.getAbbrev(), context.formatPhrase(RenderingI18nContext.STRUC_DEF_BLACK), ss.getColor(), context.getWorker().getSpecUrl()+"versions.html#std-process", true); 
    } 
 
    ExtensionContext extDefn = null; 
    if (ext) { 
      if (element != null) { 
        if (element.getTypeList().size() == 1 && element.getTypeList().get(0).hasProfile()) { 
          String eurl = element.getTypeList().get(0).getProfileList().get(0).getValue(); 
          extDefn = locateExtension(StructureDefinition.class, eurl, profile);
          if (extDefn == null) { 
            res.add(genCardinality(gen, element, row, hasDef, used, null)); 
            res.add(addCell(row, gen.new Cell(null, null, "?gen-e1? "+element.getTypeList().get(0).getProfileList(), null, null))); 
...
//          disable 26-02-2025 GDG - this just makes things inconsistent, and why do this?  nameCell.getPieces().get(0).setText(name); 
            ...
            nameCell.getPieces().get(0).setHint((context.formatPhrase(RenderingI18nContext.STRUC_DEF_EX_URL, extDefn.getUrl()))); 
            ...
            if (valueDefn != null && !"0".equals(valueDefn.getMax())) 
              ...
              // genTypes(gen, row, extDefn.getSnapshot().getElementList().get(0), profileBaseFileName, profile); 
              res.add(addCell(row, gen.new Cell(null, null, "("+(context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMPLEX))+")", null, null))); 
    ...
    if (max.hasValue() && min.hasValue() && "*".equals(max.getValue()) && 0 == min.getValue()) { 
      if (definition.hasExtension(ExtensionDefinitions.EXT_JSON_EMPTY)) { 
        ...
        if ("present".equals(code)) { 
          hint = context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_IS); 
        } else { 
          hint = context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_MAY);           
        } 
      } 
    } 
  ...
  private Row makeChoiceElementRow(HierarchicalTableGenerator gen, Row prow, ElementChoiceGroup grp, ElementDefinition parent, boolean isConstraintMode) { 
    ...
    row.setLineColor(1); 
    row.setIcon("icon_choice.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_CHOICE)); 
    row.getCells().add(gen.new Cell(null, null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CHOICE), "", null)); 
  ...
  private boolean standardExtensionSlicing(ElementDefinition element) { 
    ...
    return (t.equals("extension") || t.equals("modifierExtension")) 
        && element.getSlicing().getRules() != SlicingRules.CLOSED && element.getSlicing().getDiscriminatorList().size() == 1 && element.getSlicing().getDiscriminatorList().get(0).getPath().equals("url") && element.getSlicing().getDiscriminatorList().get(0).getType().equals(DiscriminatorType.VALUE); 
  } 
  ...
  public Cell generateDescription(RenderingStatus status, HierarchicalTableGenerator gen, Row row, ElementDefinition definition, ElementDefinition fallback, String baseURL, String url, StructureDefinition profile, String corePath, String imagePath, boolean root, boolean logicalModel, boolean allInvariants, ElementDefinition valueDefn, boolean snapshot, boolean mustSupportOnly, boolean allowSubRows, RenderingContext rc, List<ElementDefinition> inScopeElements, ResourceWrapper res) throws IOException, FHIRException {
    ...
    if (logicalModel && ExtensionUtilities.hasAnyOfExtensions(profile, ExtensionDefinitions.EXT_XML_NAMESPACE, ExtensionDefinitions.EXT_XML_NAMESPACE_DEPRECATED)) {
      if (root) {
        c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_XML_NAME)) + ": ", null).addStyle("font-weight:bold"));
    ...
    if (root) {
      if (profile != null && profile.getAbstract()) {
        ...
        c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ABSTRACT) + " " + (profile.getDerivation() == TypeDerivationRule.CONSTRAINT ? "profile" : "type") + ". ", null));
        ...
        if (!children.isEmpty()) {
          c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CHILD) + " " + (profile.getDerivation() == TypeDerivationRule.CONSTRAINT ? "profiles" : "types") + ": ", null));
          boolean first = true;
    ...
    if (definition.getPath().endsWith("url") && definition.hasFixed()) {
      ...
      if (url != null) {
        ...
        if (fixedUrl == null) {
          if (!Utilities.noString(fullUrl)) {
            c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.GENERAL_URL)) + ": ", null).addStyle("font-weight:bold"));
          ...
          c.getPieces().add(gen.new Piece(null, (" " + context.formatPhrase(RenderingI18nContext.STRUC_DEF_PROFILED) + " ") + " ", null).addStyle("font-weight:bold"));
          c.getPieces().add(gen.new Piece(ref, fullUrl, null));
      ...
      if (root) {
        if (profile.getDerivation() == TypeDerivationRule.SPECIALIZATION) {
          ...
          if (ancestors.size() > 0) {
            c.addPiece(gen.new Piece("br"));
            c.addPiece(gen.new Piece("br"));
            c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ELEMENTS), null));
            boolean first = true;
        ...
        if (profile.hasExtension(ExtensionDefinitions.EXT_RESOURCE_IMPLEMENTS)) {
          ...
          c.getPieces().add(gen.new Piece(spec("uml.html#interfaces"), context.formatPhrase(RenderingI18nContext.STRUC_DEF_IMPLEMENTS), null));
          boolean first = true;
        ...
        if (profile.hasExtension(ExtensionDefinitions.EXT_TYPE_CHARACTERISTICS) && profile.getKind() == StructureDefinitionKind.RESOURCE) {
          ...
          c.addPiece(gen.new Piece("https://hl7.org/fhir/extensions/ValueSet-type-characteristics-code.html#expansion", context.formatPhrase(RenderingI18nContext.STRUC_DEF_TYPE_CHARACTERISTICS), null));
          c.addPiece(gen.new Piece(null,  ": ", null));
          boolean first = true;
      ...
      if (definition.hasSlicing()) {
        ...
        c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_SLICE)) + ": ", null).addStyle("font-weight:bold"));
      ...
      if (!definition.getPath().contains(".") && ExtensionUtilities.hasExtension(profile, ExtensionDefinitions.EXT_BINDING_STYLE)) {
        ...
        c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.GENERAL_BINDING)) + ": ", null).addStyle("font-weight:bold"));
        c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_TYPE_SET) + " "), null));
        ...
        c.getPieces().add(gen.new Piece(null, " " + context.formatPhrase(RenderingI18nContext.STRUC_DEF_BINDING_STYLE), null));
      }
      if (definition.hasValueAlternatives()) {
        addCanonicalList(gen, c, definition.getValueAlternativesList(), "The primitive value may be replaced by the extension", true, profile);
      }
      if (definition.hasExtension(ExtensionDefinitions.EXT_IMPLIED_PREFIX)) {
        if (!c.getPieces().isEmpty()) {
          c.addPiece(gen.new Piece("br"));
        }
        c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_ELE_READ) + " "), null));
        ...
        c.getPieces().add(gen.new Piece(null, " " + (context.formatPhrase(RenderingI18nContext.STRUC_DEF_PREFIXED)), null));
      }
      if (root && ProfileUtilities.isModifierExtension(profile)) {
        if (!c.getPieces().isEmpty()) {
          c.addPiece(gen.new Piece("br"));
        }
        c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MODIFIER_EXT), null).addStyle("font-weight:bold"));
      ...
      if (definition.hasExtension(ExtensionDefinitions.EXT_EXTENSION_STYLE_NEW, ExtensionDefinitions.EXT_EXTENSION_STYLE_DEPRECATED)) {
        ...
        if ("named-elements".equals(es)) {
          if (rc.hasLink(KnownLinkType.JSON_NAMES)) {
            c.getPieces().add(gen.new Piece(rc.getLink(KnownLinkType.JSON_NAMES, true), context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT_JSON), null));
          } else {
            c.getPieces().add(gen.new Piece(ExtensionDefinitions.WEB_EXTENSION_STYLE, context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT_JSON), null));
          }
        }
      }
      ...
      if (definition.hasExtension(ExtensionDefinitions.EXT_DATE_FORMAT)) {
        ...
        if (df != null) {
          ...
          c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_DATE, df) + " "), null));
        }
      }
      if (definition.hasExtension(ExtensionDefinitions.EXT_ID_EXPECTATION)) {
        ...
        if (ide.equals("optional")) {
          ...
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_IS), null));
        } else if (ide.equals("required")) {
          if (!c.getPieces().isEmpty()) {
            c.addPiece(gen.new Piece("br"));
          }
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_MAY), null));
        } else if (ide.equals("required")) {
          if (!c.getPieces().isEmpty()) {
            c.addPiece(gen.new Piece("br"));
          }
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_NOT_ALLOW), null));
        }
      }
      if (definition.hasExtension(ExtensionDefinitions.EXT_ID_CHOICE_GROUP)) {
        ...
        c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_CHOICE_GRP)) + ": ", null).addStyle("font-weight:bold"));
        c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_REPEAT), null));
      }
      if (definition.hasExtension(ExtensionDefinitions.EXT_XML_NAME, ExtensionDefinitions.EXT_XML_NAME_DEPRECATED)) {
        ...
        if (definition.hasExtension(ExtensionDefinitions.EXT_XML_NAMESPACE, ExtensionDefinitions.EXT_XML_NAMESPACE_DEPRECATED)) {
          c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.GENERAL_XML)) + ": ", null).addStyle("font-weight:bold"));
          ...
          c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_XML_ELE)) + ": ", null).addStyle("font-weight:bold"));
          c.getPieces().add(gen.new Piece(null, definition.getExtensionString(ExtensionDefinitions.EXT_XML_NAME, ExtensionDefinitions.EXT_XML_NAME_DEPRECATED), null));
        }
      ...
      if (definition.hasExtension(ExtensionDefinitions.EXT_JSON_EMPTY)) {
        ...
        String code = ExtensionUtilities.readStringExtension(definition, ExtensionDefinitions.EXT_JSON_EMPTY);
        if ("present".equals(code)) {
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_INFERRED), null));
        } else {
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_ARRAY), null));
        }
      }
      ...
      if (!Utilities.noString(jn)) {
        ...
        if (definition.getPath().contains(".")) {
          c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_NAME)) + ": ", null).addStyle("font-weight:bold"));
          c.getPieces().add(gen.new Piece(null, jn, null));
        } else {
          c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_TYPE)) + ": ", null).addStyle("font-weight:bold"));
          Piece piece = gen.new Piece("code");
      ...
      if (ExtensionUtilities.readBoolExtension(definition, ExtensionDefinitions.EXT_JSON_NULLABLE)) {
        ...
        c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_NULL), null));
      }
      if (definition.hasExtension(ExtensionDefinitions.EXT_JSON_PROP_KEY)) {
        ...
        c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_SINGLE_JSON_OBJECTS, code), null));
      }
      if (definition.hasExtension(ExtensionDefinitions.EXT_TYPE_SPEC)) {
        for (Extension e : definition.getExtensionsByUrl(ExtensionDefinitions.EXT_TYPE_SPEC)) {
          ...
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_IF), null));
          Piece piece = gen.new Piece("code");
          piece.addHtml(new XhtmlNode(NodeType.Text).setContent(cond));
          c.getPieces().add(piece);
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_THEN_TYPE) + " ", null));
      ...
      if (root) {
        if (ExtensionUtilities.readBoolExtension(profile, ExtensionDefinitions.EXT_OBLIGATION_PROFILE_FLAG_NEW, ExtensionDefinitions.EXT_OBLIGATION_PROFILE_FLAG_OLD)) {
          ...
          c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_OBLIG_ADD), null).addStyle("font-weight:bold"));
        }
        ...
        if (profile.getKind() == StructureDefinitionKind.LOGICAL) {
          ...
          if (canBeTarget == null) {
            ...
            c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CAN_TARGET), null).addStyle("font-weight:bold"));
          } else {
            if (!c.getPieces().isEmpty()) {
              c.addPiece(gen.new Piece("br"));
            }
            c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_NOT_MARK), null).addStyle("font-weight:bold"));
          }
          ...
          if (ps != null) {
            ...
            if ("cda".equals(ps)) {
              c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_TEMPLATEID), null).addStyle("font-weight:bold"));
            } else {
              c.addPiece(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNKNOWN_APPROACH, ps) + " ", null).addStyle("font-weight:bold"));
            }
          }
          ...
          if (lc != null && lc.hasValueUriType()) {
            ...
            c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL_CONT)) + ": ", context.formatPhrase(RenderingI18nContext.STRUC_DEF_ROOT)).addStyle("font-weight:bold"));
      ...
      if (definition != null) {
        ...
        if (binding != null && !binding.isEmpty()) {
          ...
          if (!binding.hasValueSet()) {
            c.getPieces().add(checkForNoChange(binding, gen.new Piece("binding", null, (context.formatPhrase(RenderingI18nContext.GENERAL_BINDING_NO_VS_1)) + ": ", null).addStyle("font-weight:bold")));
            if (binding.hasStrength()) {
              ...
              if (binding.getStrength().testable()) {
                ...
                c.getPieces().add(checkForNoChange(binding, gen.new Piece("binding", null, "\u26A0", context.formatPhrase(RenderingI18nContext.GENERAL_BINDING_NO_VS_2))).addStyle("font-weight:bold; color: #c97a18"));
              }
            }
            c.getPieces().add(gen.new Piece("binding", null, ": ", null));
            ...
            c.getPieces().add(checkForNoChange(binding, gen.new Piece("binding", null, (context.formatPhrase(RenderingI18nContext.GENERAL_BINDING)) + ": ", null).addStyle("font-weight:bold")));
        ...
        for (ElementDefinitionConstraintComponent inv : definition.getConstraintList()) {
//            if (!inv.hasSource() || profile == null || inv.getSource().equals(profile.getUrl()) || allInvariants) { 
        ...
        if ((definition.hasBase() && "*".equals(definition.getBase().getMax())) || (definition.hasMax() && "*".equals(definition.getMax()))) {
          ...
          if (definition.hasOrderMeaning()) {
            c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_REPEAT_ELE, definition.getOrderMeaning()), null));
          } else {
        ...
        if (definition.hasFixed()) {
          ...
          c.getPieces().add(checkForNoChange(definition.getFixed(), gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED)) + ": ", null).addStyle("font-weight:bold")));
          if (!useTableForFixedValues || !allowSubRows || definition.getFixed().isPrimitive()) {
            ...
            c.getPieces().add(checkForNoChange(definition.getFixed(), gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_AS_SHOWN), null).addStyle("color: darkgreen")));
          ...
          c.getPieces().add(checkForNoChange(definition.getPattern(), gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_REQ_PATT)) + ": ", null).addStyle("font-weight:bold")));
          if (!useTableForFixedValues || !allowSubRows || definition.getPattern().isPrimitive())
            ...
            c.getPieces().add(checkForNoChange(definition.getPattern(), gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LEAST_FOLLOW), null).addStyle("color: darkgreen")));
          ...
          c.getPieces().add(gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED_VALUE)) + " ", null).addStyle("font-weight:bold"));
          ...
          for (ElementDefinitionExampleComponent ex : definition.getExampleList()) {
            if (!c.getPieces().isEmpty()) {
              c.addPiece(gen.new Piece("br"));
            }
            c.getPieces().add(checkForNoChange(ex, gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.GENERAL_EXAMPLE)) + ("".equals("General") ? "" : " " + ex.getLabel()) + ": ", null).addStyle("font-weight:bold")));
        ...
        if (definition.hasMaxLength() && definition.getMaxLength() != 0) {
          ...
          c.getPieces().add(checkForNoChange(definition.getMaxLengthElement(), gen.new Piece(null, context.formatPhrase(RenderingI18nContext.GENERAL_MAX_LENGTH), null).addStyle("font-weight:bold")));
        ...
        if (definition.hasExtension(ExtensionDefinitions.EXT_MIN_LENGTH)) {
          ...
          if (min > 0) {
            ...
            c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.GENERAL_MIN_LENGTH), null).addStyle("font-weight:bold"));
            c.getPieces().add(gen.new Piece(null, Integer.toString(min), null).addStyle("color: darkgreen"));
          }
        }
        if (profile != null) {
          for (StructureDefinitionMappingComponent md : profile.getMappingList()) {
            if (md.hasExtension(ExtensionDefinitions.EXT_TABLE_NAME)) {
              ...
              for (ElementDefinitionMappingComponent m : definition.getMappingList())
                if (m.getIdentity().equals(md.getIdentity()))
                  map = m;
              if (map != null) {
                for (int i = 0; i < definition.getMappingList().size(); i++) {
                  c.addPiece(gen.new Piece("br"));
  ...
  private Set<String> determineNarrativeStatus(ElementDefinition definition, StructureDefinition profile, boolean snapshot) {
    ...
    try {
      ...
      if (status != null && status.getBinding().hasValueSet()) {
        ...
        if (exp.isOk()) {
          for (ValueSetExpansionContainsComponent cc : exp.getValueset().getExpansion().getContainsList()) {
            set.add(cc.getCode());
          }
        }
      }
  ...
  private boolean isAttr(SourcedElementDefinition ed) { 
    for (Enumeration<PropertyRepresentation> t : ed.getDefinition().getRepresentationList()) { 
  ...
  private void getAncestorElements(List<String> inscope, StructureDefinition profile, List<SourcedElementDefinition> ancestors) { 
    ...
    if (base != null) { 
      ...
      for (ElementDefinition ed : base.getDifferential().getElementList()) { 
  ...
  private Cell genTypes(HierarchicalTableGenerator gen, Row r, ElementDefinition e, String profileBaseFileName, StructureDefinition profile, String corePath, String imagePath, boolean root, boolean mustSupportMode, boolean diff) {
    ...
    if (e.hasContentReference()) { 
      ElementInStructure ed = getElementByName(diff ? profile.getDifferential().getElementList() : profile.getSnapshot().getElementList(), e.getContentReference(), profile);
      if (ed == null) 
        c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNKNOWN_REF, e.getContentReference())+" ", null)); 
      else { 
    ...
    for (TypeRefComponent t : types) { 
      if (!mustSupportMode || allTypesMustSupport(e) || isMustSupport(t)) { 
        ...
        if (t.hasTarget()) { 
          if (t.hasProfile()) { 
            String ref = t.getProfileList().get(0).asStringValue(); 
            StructureDefinition tsd = context.getProfileUtilities().findProfile(t.getProfileList().get(0), profile);
            if (tsd != null) { 
          ...
          for (CanonicalType u : t.getTargetProfileList()) { 
            if (!mustSupportMode || allProfilesMustSupport(t.getTargetProfileList()) || isMustSupport(u)) { 
              if (tfirst) 
                tfirst = false; 
              else 
                c.addPiece(gen.new Piece(null, " | ", null)); 
          ...
          c.getPieces().add(gen.new Piece(null, ")", null)); 
          if (t.getAggregationList().size() > 0) { 
        ...
        } else if (t.hasProfile() && (!t.getWorkingCode().equals("Extension") || isProfiledType(t.getProfileList()))) { // a profiled type 
          String ref; 
          boolean pfirst = true; 
          for (CanonicalType p : t.getProfileList()) { 
            if (!mustSupportMode || allProfilesMustSupport(t.getProfileList()) || isMustSupport(p)) { 
              if (pfirst) { 
                pfirst = false; 
              } else { 
                c.addPiece(checkForNoChange(tl, gen.new Piece(null,", ", null))); 
  ...
  private void genGridElement(String defPath, HierarchicalTableGenerator gen, List<Row> rows, ElementDefinition element, List<ElementDefinition> all, List<StructureDefinition> profiles, boolean showMissing, String profileBaseFileName, Boolean extensions, String corePath, String imagePath, boolean root, boolean isConstraintMode, boolean diff) throws IOException, FHIRException {
    ...
    if (!onlyInformationIsMapping(all, element)) { 
      ...
      if (element.getTypeList().size() == 1 && element.getTypeList().get(0).isPrimitive()) 
      ...
      /*      if (element.hasSlicing()) { 
        if (standardExtensionSlicing(element)) { 
          used.used = element.hasType() && element.getTypeList().get(0).hasProfile(); 
          showMissing = false; 
        } else { 
          row.setIcon("icon_slice.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_SLICE); 
  ...
  private ExtensionContext locateExtension(Class<StructureDefinition> class1, String value, StructureDefinition sd)  {
    if (value.contains("#")) { 
      ...
      for (ElementDefinition ted : ext.getSnapshot().getElementList()) { 
      ...
      if (ext == null) 
        ...
        return new ExtensionContext(ext, ext.getSnapshot().getElementList().get(0)); 
    } 
  } 
  ...
  private boolean extensionIsComplex(String value, StructureDefinition sd) {
    if (value.contains("#")) { 
      ...
      if (ed == null) 
        return false; 
      int i = ext.getSnapshot().getElementList().indexOf(ed); 
      int j = i+1; 
      while (j < ext.getSnapshot().getElementList().size() && !ext.getSnapshot().getElementList().get(j).getPath().equals(ed.getPath())) 
        j++; 
      return j - i > 5; 
    } else { 
      StructureDefinition ext = context.getProfileUtilities().findProfileStr(value, sd);
      return ext != null && ext.getSnapshot().getElementList().size() > 5; 
    } 
  } 
  ...
  private BindingResolution makeNullBr(ElementDefinitionBindingComponent binding) { 
    ...
    br.display = context.formatPhrase(RenderingI18nContext.GENERAL_TODO); 
    return br; 
  } 
  ...
  private ElementDefinitionBindingComponent makeUnifiedBinding(ElementDefinitionBindingComponent binding, ElementDefinition element) { 
    ...
    // todo: derivation? 
    b.getExtensionList().addAll(binding.getExtensionList()); 
    return b; 
  } 
  ...
  private void genFixedValue(HierarchicalTableGenerator gen, Row erow, DataType value, boolean snapshot, boolean pattern, String corePath, boolean skipnoValue, String path, String id, List<ElementDefinition> inScopeElements) { 
    ...
    for (org.hl7.fhir.model.Property t : value.getChildren()) {
          ...
          if (t.getValues().size() == 0 || (t.getValues().size() == 1 && t.getValues().get(0).isEmpty())) { 
            if (!skipnoValue) { 
              ...
              if (!pattern) { 
                ...
                row.setIcon("icon_fixed.gif", context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED_VALUE) /*context.formatPhrase(RenderingI18nContext.TEXT_ICON_FIXED*/); 
            ...
            for (Base b : t.getValues()) { 
              ...
              row.setId(context.prefixAnchor(ed.getPath())); 
              erow.getSubRows().add(row); 
              row.setIcon("icon_fixed.gif", context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED) /*context.formatPhrase(RenderingI18nContext.TEXT_ICON_FIXED*/); 
              ...
              if (b.isPrimitive()) { 
                ...
                c.addPiece(gen.new Piece("br")); 
                c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED_VALUE)+" ", null).addStyle("font-weight: bold")); 
                c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMPLEXBRACK), null).addStyle("color: darkgreen")); 
  ...
  private String formatMergedPatternValue(Base b) {
    ...
    if (b instanceof DataType) {
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMPLEXBRACK);
    }
  ...
  private Cell generateGridDescription(HierarchicalTableGenerator gen, Row row, ElementDefinition definition, ElementDefinition fallback, boolean used, String baseURL, String url, StructureDefinition profile, String corePath, String imagePath, boolean root, ElementDefinition valueDefn) throws IOException, FHIRException { 
    ...
    if (used) { 
      if (definition.hasContentReference()) { 
        ElementInStructure ed = getElementByName(profile.getSnapshot().getElementList(), definition.getContentReference(), profile); 
        if (ed == null) 
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNKNOWN_REF, definition.getContentReference()), null)); 
        else { 
          if (ed.getSource() == profile) { 
      ...
      if (definition.getPath().endsWith("url") && definition.hasFixed()) { 
        ...
        if (url != null) { 
          ...
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_URLS), null).addStyle("font-weight:bold")); 
          c.getPieces().add(gen.new Piece(ref, fullUrl, null)); 
        } 
 
        if (definition.hasSlicing()) { 
          if (!c.getPieces().isEmpty()) { c.addPiece(gen.new Piece("br")); } 
          c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_SLICES), null).addStyle("font-weight:bold")); 
          c.getPieces().add(gen.new Piece(null, describeSlice(definition.getSlicing()), null)); 
        } 
        if (definition != null) { 
          ElementDefinitionBindingComponent binding = null; 
          ...
          if (binding!=null && !binding.isEmpty()) { 
            ...
            c.getPieces().add(checkForNoChange(binding, gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_BINDINGS), null).addStyle("font-weight:bold"))); 
          ...
          if (definition.hasFixed()) { 
            if (!c.getPieces().isEmpty()) { c.addPiece(gen.new Piece("br")); } 
            c.getPieces().add(checkForNoChange(definition.getFixed(), gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED_VALUE), null).addStyle("font-weight:bold"))); 
            String s = buildJson(definition.getFixed()); 
            String link = null; 
            if (Utilities.isAbsoluteUrl(s)) 
              link = context.getPkp().getLinkForUrl(corePath, s); 
            ...
            if (!c.getPieces().isEmpty()) { c.addPiece(gen.new Piece("br")); } 
            c.getPieces().add(checkForNoChange(definition.getPattern(), gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_REQUIRED_PATT), null).addStyle("font-weight:bold"))); 
            c.getPieces().add(checkForNoChange(definition.getPattern(), gen.new Piece(null, buildJson(definition.getPattern()), null).addStyle("color: darkgreen"))); 
          } else if (definition.hasExample()) { 
            for (ElementDefinitionExampleComponent ex : definition.getExampleList()) { 
              if (!c.getPieces().isEmpty()) { c.addPiece(gen.new Piece("br")); } 
              c.getPieces().add(checkForNoChange(ex, gen.new Piece(null, context.formatPhrase(RenderingI18nContext.GENERAL_EXAMPLE) +"'"+("".equals("General")? "": " "+ex.getLabel()+"'")+": ", "").addStyle("font-weight:bold"))); 
              c.getPieces().add(checkForNoChange(ex, gen.new Piece(null, buildJson(ex.getValue()), null).addStyle("color: darkgreen"))); 
            } 
          } 
          if (definition.hasMaxLength() && definition.getMaxLength()!=0) { 
          ...
          if (definition.hasDefinition()) { 
            if (!c.getPieces().isEmpty()) { c.addPiece(gen.new Piece("br")); } 
            c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.GENERAL_DEFINITION_COLON), null).addStyle("font-weight:bold")); 
            c.addPiece(gen.new Piece("br")); 
          ...
          if (definition.getComment()!=null) { 
            if (!c.getPieces().isEmpty()) { c.addPiece(gen.new Piece("br")); } 
            c.getPieces().add(gen.new Piece(null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMMENT), null).addStyle("font-weight:bold")); 
            c.addPiece(gen.new Piece("br")); 
  ...
  private boolean onlyInformationIsMapping(ElementDefinition d) { 
    return !d.hasShort() && !d.hasDefinition() && 
        !d.hasRequirements() && !d.getAliasList().isEmpty() && !d.hasMinElement() && 
        !d.hasMax() && !d.getTypeList().isEmpty() && !d.hasContentReference() && 
        !d.hasExample() && !d.hasFixed() && !d.hasMaxLengthElement() && 
        !d.getConditionList().isEmpty() && !d.getConstraintList().isEmpty() && !d.hasMustSupportElement() && 
        !d.hasBinding(); 
  } 
  ...
  public String listConstraintsAndConditions(ElementDefinition element) { 
    ...
    for (ElementDefinitionConstraintComponent con : element.getConstraintList()) { 
    ...
    for (IdType id : element.getConditionList()) { 
    ...
    for (TypeRefComponent tr : element.getTypeList()) { 
      if (!mustSupportMode || allTypesMustSupport(element) || isMustSupport(tr)) { 
        ...
        if (isReference(tu)) { 
          ...
          choicerow.setIcon("icon_reference.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_REFERENCE)); 
          Cell c = gen.new Cell(); 
          ...
          boolean first = true; 
          for (CanonicalType rt : tr.getTargetProfileList()) { 
          ...
          if (sd == null) { 
            ...
            choicerow.getCells().add(gen.new Cell()); 
            choicerow.getCells().add(gen.new Cell(null, null, "", null, null)); 
            choicerow.setIcon("icon_primitive.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_PRIMITIVE)); 
            ...
            choicerow.getCells().add(gen.new Cell()); 
            choicerow.getCells().add(gen.new Cell(null, null, "", null, null)); 
            choicerow.setIcon("icon_datatype.gif", context.formatPhrase(RenderingI18nContext.TEXT_ICON_DATATYPE)); 
          ...
          if (tr.hasProfile() && used) { 
            ...
            boolean first = true; 
            for (CanonicalType pt : tr.getProfileList()) { 
              if (!mustSupportMode || allProfilesMustSupport(tr.getProfileList()) || isMustSupport(pt)) { 
  ...
  private String describeSlice(ElementDefinitionSlicingComponent slicing) { 
    return formatPhrase(RenderingContext.SD_SLICING_INFO, slicing.getOrdered() ? (context.formatPhrase(RenderingI18nContext.STRUC_DEF_ORDERED)) : (context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNORDERED)), describe(slicing.getRules()), commas(slicing.getDiscriminatorList())); 
  } 
  ...
  private String describe(SlicingRules rules) { 
    if (rules == null) 
      return (context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNSPECIFIED)); 
    switch (rules) { 
    case CLOSED : return (context.formatPhrase(RenderingI18nContext.STRUC_DEF_CLOSED)); 
    case OPEN : return (context.formatPhrase(RenderingI18nContext.STRUC_DEF_OPEN)); 
    case OPENATEND : return (context.formatPhrase(RenderingI18nContext.STRUC_DEF_OPEN_END)); 
    default: 
      return "?gen-sr?"; 
    } 
  } 
  ...
  public boolean isMustSupport(TypeRefComponent tr) { 
    ...
    if (isMustSupport(tr.getProfileList())) { 
      return true; 
    } 
    return isMustSupport(tr.getTargetProfileList()); 
  } 
  ...
  private SpanEntry buildSpanEntryFromProfile(String name, String cardinality, StructureDefinition profile) throws IOException { 
    ...
    if (profile.getDerivation() == TypeDerivationRule.CONSTRAINT) { 
      ...
      res.setDescription(context.formatPhrase(RenderingI18nContext.STRUC_DEF_FHIR, profile.getName())+" "); 
  ...
  private String summarizeCoding(Coding value) { 
    ...
    if (Utilities.isURL(system)) { 
      if (system.equals("http://cap.org/protocols")) 
        system = context.formatPhrase(RenderingI18nContext.STRUC_DEF_CAP); 
    } 
  ...
  private TableModel initSpanningTable(HierarchicalTableGenerator gen, String prefix, boolean isLogical, String id) throws IOException { 
    ...
    model.getTitles().add(gen.new Title(null, model.getDocoRef(), context.formatPhrase(RenderingI18nContext.STRUC_DEF_PROPERTY), context.formatPhrase(RenderingI18nContext.STRUC_DEF_PROF_RES), null, 0)); 
    model.getTitles().add(gen.new Title(null, model.getDocoRef(), context.formatPhrase(RenderingI18nContext.GENERAL_CARD), context.formatPhrase(RenderingI18nContext.STRUC_DEF_MAX_MIN), null, 0)); 
    model.getTitles().add(gen.new Title(null, model.getDocoRef(), context.formatPhrase(RenderingI18nContext.GENERAL_CONTENT), context.formatPhrase(RenderingI18nContext.STRUC_DEF_WHAT), null, 0)); 
    model.getTitles().add(gen.new Title(null, model.getDocoRef(), context.formatPhrase(RenderingI18nContext.GENERAL_DESC), context.formatPhrase(RenderingI18nContext.STRUC_DEF_DESC_PROF), null, 0)); 
    return model; 
  } 
  ...
  private void genSpanEntry(HierarchicalTableGenerator gen, List<Row> rows, SpanEntry span) throws IOException { 
    ...
    //row.setColor(..?); 
    if (span.isProfile()) { 
      row.setIcon("icon_profile.png", context.formatPhrase(RenderingI18nContext.GENERAL_PROF)); 
    } else { 
      row.setIcon("icon_resource.png", context.formatPhrase(RenderingI18nContext.GENERAL_RESOURCE)); 
    } 
  ...
  private SpanEntry buildSpanningTable(String name, String cardinality, StructureDefinition profile, Set<String> processed, boolean onlyConstraints, String constraintPrefix) throws IOException { 
    ...
    if (wantProcess && profile.getDerivation() == TypeDerivationRule.CONSTRAINT) { 
      for (ElementDefinition ed : profile.getSnapshot().getElementList()) { 
        if (!"0".equals(ed.getMax()) && ed.getTypeList().size() > 0) { 
          String card = getCardinality(ed, profile.getSnapshot().getElementList()); 
          if (!card.endsWith(".0")) { 
  ...
  public XhtmlNode formatTypeSpecifiers(ElementDefinition d) { 
    ...
    for (Extension e : d.getExtensionsByUrl(ExtensionDefinitions.EXT_TYPE_SPEC)) { 
      ...
      String cond = ExtensionUtilities.readStringExtension(e, "condition"); 
      String type = ExtensionUtilities.readStringExtension(e, "type"); 
      x.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_IF)+" "); 
      x.code().tx(cond); 
      x.tx(" "+(context.formatPhrase(RenderingI18nContext.STRUC_DEF_THEN_TYPE)+" ")); 
  ...
  public XhtmlNode generateExtensionTable(RenderingStatus status, String defFile, StructureDefinition ed, String imageFolder, boolean inlineGraphics, boolean full, String corePath, String imagePath, Set<String> outputTracker, RenderingContext rc, String defPath, String anchorPrefix, ResourceWrapper res, boolean diff) throws IOException, FHIRException {
    ...
    if (ed.getSnapshot().getElementFirstRep().getIsModifier()) 
      m = "modifier_"; 
    for (ElementDefinition eld : ed.getSnapshot().getElementList()) { 
    ...
    if (!full) 
      en = ed.getName(); 
    else if (ed.getSnapshot().getElementList().get(0).getIsModifier()) 
    ...
    r.getCells().add(gen.new Cell(null, null, describeCardinality(ed.getSnapshot().getElementList().get(0), null, new UnusedTracker()), null, null)); 
 
    ElementDefinition ved = null; 
    if (full || vdeep) { 
      r.getCells().add(gen.new Cell("", "", "Extension", null, null)); 
 
      r.setIcon(deep ? "icon_"+m+"extension_complex.png" : "icon_extension_simple.png", deep ? context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_COMPLEX) : context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE)); 
      ...
      for (ElementDefinition child : children) 
        if (!child.getPath().endsWith(".id")) { 
          ...
          sdl.add(ed); 
          genElement(status, defFile == null ? "" : defFile+"-definitions.html#extension.", gen, r.getSubRows(), child, ed.getSnapshot().getElementList(), sdl, true, defFile, true, full, corePath, imagePath, true, false, false, false, null, false, rc, "", ed, null, res, false, null, diff);
        } 
    } else if (deep) { 
      List<ElementDefinition> children = new ArrayList<ElementDefinition>(); 
      for (ElementDefinition ted : ed.getSnapshot().getElementList()) { 
        if (ted.getPath().equals("Extension.extension")) 
          children.add(ted); 
      } 
 
      r.getCells().add(gen.new Cell("", "", "Extension", null, null)); 
      r.setIcon("icon_"+m+"extension_complex.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_COMPLEX)); 
      ...
      for (ElementDefinition c : children) { 
        ...
        if (ved != null && ued != null) { 
          ...
          r1.setIcon("icon_"+m+"extension_simple.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE));       
      ...
      r.setIcon("icon_"+m+"extension_simple.png", context.formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE));       
    } 
    ...
    if (!full && !(deep || vdeep) && ved != null && ved.hasBinding()) {   
      ...
      c.getPieces().add(checkForNoChange(ved.getBinding(), gen.new Piece(null, (context.formatPhrase(RenderingI18nContext.GENERAL_BINDING))+": ", null).addStyle("font-weight:bold"))); 
  ...
  private ElementDefinition getValueFor(StructureDefinition ed, ElementDefinition c) { 
    int i = ed.getSnapshot().getElementList().indexOf(c) + 1; 
    while (i < ed.getSnapshot().getElementList().size() && ed.getSnapshot().getElementList().get(i).getPath().startsWith(c.getPath()+".")) { 
      if (ed.getSnapshot().getElementList().get(i).getPath().startsWith(c.getPath()+".value")) 
        return ed.getSnapshot().getElementList().get(i); 
      i++; 
    } 
    return null; 
  } 
  ...
  private ElementDefinition getUrlFor(StructureDefinition ed, ElementDefinition c) { 
    int i = ed.getSnapshot().getElementList().indexOf(c) + 1; 
    while (i < ed.getSnapshot().getElementList().size() && ed.getSnapshot().getElementList().get(i).getPath().startsWith(c.getPath()+".")) { 
      if (ed.getSnapshot().getElementList().get(i).getPath().equals(c.getPath()+".url")) 
        return ed.getSnapshot().getElementList().get(i); 
      i++; 
    } 
    return null; 
    ...
    for (ElementDefinition ec : elements) { 
      if ((incProfiledOut || !"0".equals(ec.getMax())) && !excluded.contains(ec)) { 
        ...
        if (isProfiledExtension(ec)) { 
          StructureDefinition extDefn = context.getProfileUtilities().findProfile(ec.getTypeList().get(0).getProfileList().get(0), sd);
          if (extDefn == null) { 
            ...
            try { 
              StructureDefinition compareExtDefn = context.getProfileUtilities().findProfile(compareElement.getTypeList().get(0).getProfileList().get(0), sd);
            ...
            // generateElementInner(b, extDefn, extDefn.getSnapshot().getElementList().get(0), valueDefn == null ? 2 : 3, valueDefn); 
          } 
        } else { 
  ...
  public ElementDefinition getElementById(String url, String id, StructureDefinition srcSd) {
    ...
    if (sdCache == null) { 
      ...
      if (sd == null) { 
        ...
        if (sd == null) { 
          throw new FHIRException(context.formatPhrase(RenderingI18nContext.STRUC_DEF_FHIR_EXCEP, url)+" "); 
        } 
      } 
...
//    int slicedExtensionMode = (mode == GEN_MODE_KEY) && slicedExtension ? GEN_MODE_SNAP : mode; // see ProfileUtilities.checkExtensionDoco / Task 3970 
    if (d.hasSliceName()) { 
      ...
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CONSTRAINING), "profiling.html#slicing", strikethrough, compareString(encodeValue(d.getSliceIsConstrainingElement(), null), d.getSliceIsConstrainingElement(), null, (compare != null ? encodeValue(compare.getSliceIsConstrainingElement(), null) : null), d, null, "sliceName", mode, false, false));    
    } 
 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_DEFINITION), null, strikethrough, compareMarkdown(sd.getName(), d.getDefinitionElement(), (compare==null) || slicedExtension ? null : compare.getDefinitionElement(), mode)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_SHORT), null, strikethrough, compareString(d.hasShort() ? d.getShort() : null, d.getShortElement(), null, "short", d, compare!= null && compare.hasShortElement() ? compare.getShort() : null, null, mode, false, false)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_COMMENTS), null, strikethrough, compareMarkdown(sd.getName(), d.getCommentElement(), (compare==null) || slicedExtension ? null : compare.getCommentElement(), mode)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_NOTE), null, strikethrough, businessIdWarning(sd.getName(), tail(d.getPath()))); 
    ...
    if (root && sd.hasExtension(ExtensionDefinitions.EXT_TYPE_PARAMETER)) {
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_TYPE_PARAMETER), "http://hl7.org/fhir/tools/StructureDefinition-type-parameter.html", strikethrough, renderTypeParameter(sd.getExtensionByUrl(ExtensionDefinitions.EXT_TYPE_PARAMETER)));
    }
    if (d.hasExtension(ExtensionDefinitions.EXT_DEF_TYPE)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_DEFAULT_TYPE), "datatypes.html", strikethrough, ExtensionUtilities.readStringExtension(d, ExtensionDefinitions.EXT_DEF_TYPE));           
    } 
    if (d.hasExtension(ExtensionDefinitions.EXT_TYPE_SPEC)) { 
      tableRow(tbl, Utilities.pluralize(context.formatPhrase(RenderingI18nContext.STRUC_DEF_TYPE_SPEC), d.getExtensionsByUrl(ExtensionDefinitions.EXT_TYPE_SPEC).size()), "datatypes.html", strikethrough, formatTypeSpecifiers(d));           
    } 
    if (d.getPath().endsWith("[x]") && !d.prohibited()) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_NOTE_X), null, strikethrough).ahWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_SEE) 
    		  , spec("formats.html#choice"), null, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CHOICE_DATA_TYPE), context.formatPhrase(RenderingI18nContext.STRUC_DEF_FURTHER_INFO)); 
    } 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MODIFIER), "conformance-rules.html#ismodifier", strikethrough, presentModifier(d, mode, compare)); 
    if (d.getMustHaveValue()) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_PRIMITIVE), "elementdefinition.html#primitives", strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_PRIM_TYPE_VALUE)); 
    } else if (d.hasValueAlternatives()) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_PRIMITIVE), "elementdefinition.html#primitives", strikethrough, renderCanonicalList(context.formatPhrase(RenderingI18nContext.STRUC_DEF_PRIM_TYPE_PRESENT), d.getValueAlternativesList()));       
    } else if (hasPrimitiveTypes(d)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_PRIMITIVE), "elementdefinition.html#primitives", strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_PRIM_ELE));             
    } 
    if (ExtensionUtilities.hasAllowedUnits(d)) {       
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ALLOWED), "http://hl7.org/fhir/extensions/StructureDefinition-elementdefinition-allowedUnits.html", strikethrough, describeAllowedUnits(d));         
    } 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MUST_SUPPORT), "conformance-rules.html#mustSupport", strikethrough, displayBoolean(d.getMustSupport(), d.getMustSupportElement(), "mustSupport", d, compare==null ? null : compare.getMustSupportElement(), mode)); 
    if (d.getMustSupport()) { 
      if (hasMustSupportTypes(d.getTypeList())) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MUST_SUPPORT_TYPES), "datatypes.html", strikethrough, describeTypes(d.getTypeList(), true, d, compare, mode, null, null, sd)); 
      } else if (hasChoices(d.getTypeList())) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MUST_SUPPORT_TYPES), "datatypes.html", strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_NO_MUST_SUPPORT)); 
      } 
    } 
    if (root && sd.getKind() == StructureDefinitionKind.LOGICAL) { 
      ...
      if (lt == null || !lt.hasValue()) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_NOT_MARK));         
      } else if (lt.getValue().hasExtension(ExtensionDefinitions.EXT_DAR)) {         
      } else if (lt.getValueBooleanType().hasValue()) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_NOT_MARK));         
      } else if (lt.getValueBooleanType().booleanValue()) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CAN_TARGET));         
      } else { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CANNOT_TARGET)); 
      } 
      ...
      if (lc != null && lc.hasValueUriType()) { 
        ...
        StructureDefinition lct = context.getContext().fetchTypeDefinition(uri); 
        if (lct != null) { 
          tableRowLink(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL_CONT), null, strikethrough, lct.present(), lct.getWebPath());         
        } else { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOGICAL_CONT), null, strikethrough, uri);         
        } 
      } 
      ...
      if (ps != null) { 
        if ("cda".equals(ps)) { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_VALID), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_TEMPLATEID)); 
        } else { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_VALID), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNKNOWN_APPROACH, ps)+" "); 
        }               
      } 
    } 
 
    if (root && sd.hasExtension(ExtensionDefinitions.EXT_SD_IMPOSE_PROFILE)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_IMPOSE_PROFILE), "http://hl7.org/fhir/extensions/StructureDefinition-structuredefinition-imposeProfile.html", strikethrough,  
          renderCanonicalListExt(context.formatPhrase(RenderingI18nContext.STRUC_DEF_PROF_REQ)+" ", sd.getExtensionsByUrl(ExtensionDefinitions.EXT_SD_IMPOSE_PROFILE))); 
    } 
    if (root && sd.hasExtension(ExtensionDefinitions.EXT_SD_COMPLIES_WITH_PROFILE)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMP_PROF), "http://hl7.org/fhir/extensions/StructureDefinition-structuredefinition-compliesWithProfile.html", strikethrough,  
          renderCanonicalListExt(context.formatPhrase(RenderingI18nContext.STRUC_DEF_PROF_COMP)+" ", sd.getExtensionsByUrl(ExtensionDefinitions.EXT_SD_COMPLIES_WITH_PROFILE))); 
    } 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_OBLIG), null, strikethrough, describeObligations(status, d, root, sd, defPath, anchorPrefix, inScopeElements, res));
    ...
    if (d.hasExtension(ExtensionDefinitions.EXT_EXTENSION_STYLE_NEW, ExtensionDefinitions.EXT_EXTENSION_STYLE_DEPRECATED)) {
      ...
      if ("named-elements".equals(es)) { 
        if (context.hasLink(KnownLinkType.JSON_NAMES)) { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT_STYLE), context.getLink(KnownLinkType.JSON_NAMES, true), strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT_JSON)); 
        } else { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT_STYLE), ExtensionDefinitions.WEB_EXTENSION_STYLE, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_EXT_JSON));
        } 
      } 
    } 
 
    if (!d.getPath().contains(".") && ExtensionUtilities.hasExtension(sd, ExtensionDefinitions.EXT_BINDING_STYLE)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_BINDING_STYLE), ExtensionDefinitions.WEB_BINDING_STYLE, strikethrough,
    		  context.formatPhrase(RenderingI18nContext.STRUC_DEF_TYPE_BOUND, ExtensionUtilities.readStringExtension(sd, ExtensionDefinitions.EXT_BINDING_STYLE)+" binding style")+" ");             
    } 
 
    if (d.hasExtension(ExtensionDefinitions.EXT_DATE_FORMAT)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_DATE_FORM), null, strikethrough, ExtensionUtilities.readStringExtension(d, ExtensionDefinitions.EXT_DATE_FORMAT)); 
    } 
    String ide = ExtensionUtilities.readStringExtension(d, ExtensionDefinitions.EXT_ID_EXPECTATION); 
    if (ide != null) { 
      if (ide.equals("optional")) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_EXPECT), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_IS)); 
      } else if (ide.equals("required")) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_EXPECT), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_MAY)); 
      } else if (ide.equals("required")) { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_EXPECT), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ID_NOT_ALLOW)); 
      } 
    } 
 
    if (d.hasExtension(ExtensionDefinitions.EXT_ID_CHOICE_GROUP)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_CHOICE_GRP), null, strikethrough, context.formatPhrase(RenderingI18nContext.STRUC_DEF_REPEAT));           
    } 
    ...
    if (ExtensionUtilities.hasAnyOfExtensions(d, ExtensionDefinitions.EXT_JSON_EMPTY, ExtensionDefinitions.EXT_JSON_PROP_KEY, ExtensionDefinitions.EXT_JSON_NULLABLE,  
      ...
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_FORM), null, strikethrough,  describeJson(d));           
    } 
    if (d.hasExtension(ExtensionDefinitions.EXT_XML_NAMESPACE, ExtensionDefinitions.EXT_XML_NAMESPACE_DEPRECATED) || sd.hasExtension(ExtensionDefinitions.EXT_XML_NAMESPACE, ExtensionDefinitions.EXT_XML_NAMESPACE_DEPRECATED) ||  
        ...
        d.hasRepresentation()) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_XML_FORM), null, strikethrough, describeXml(sd, d, root));           
    } 
 
    if (d.hasExtension(ExtensionDefinitions.EXT_IMPLIED_PREFIX)) { 
      tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_STRING_FORM), null, strikethrough).codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_ELE_READ)+" ", ExtensionUtilities.readStringExtension(d, ExtensionDefinitions.EXT_IMPLIED_PREFIX), context.formatPhrase(RenderingI18nContext.STRUC_DEF_PREFIXED));                 
    } 
    ...
    if (d.hasExtension(ExtensionDefinitions.EXT_STANDARDS_STATUS)) { 
      ...
      if (sdb != null) { 
        ...
        if (base != null) { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_STAND_STAT), "versions.html#std-process", strikethrough, ss.toDisplay()+" (from "+base.toDisplay()+")"); 
        } else { 
          tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_STAND_STAT), "versions.html#std-process", strikethrough, ss.toDisplay());           
        } 
      } else { 
        tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_STAND_STAT), "versions.html#std-process", strikethrough, ss.toDisplay()); 
      } 
    ...
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_ALT_NAME), null, strikethrough, compareSimpleTypeLists(d.getAliasList(), ((compare==null) || slicedExtension ? null : compare.getAliasList()), mode)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_DEF_CODES), null, strikethrough, compareDataTypeLists(d.getCodeList(), ((compare==null) || slicedExtension ? null : compare.getCodeList()), mode)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MIN_VALUE), null, strikethrough, compareString(d.hasMinValue() ? encodeValue(d.getMinValue(), null) : null, d.getMinValue(), null, "minValue", d, compare!= null && compare.hasMinValue() ? encodeValue(compare.getMinValue(), null) : null, null, mode, false, false)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MAX_VALUE), null, strikethrough, compareString(d.hasMaxValue() ? encodeValue(d.getMaxValue(), null) : null, d.getMaxValue(), null, "maxValue", d, compare!= null && compare.hasMaxValue() ? encodeValue(compare.getMaxValue(), null) : null, null, mode, false, false)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_MAX_LENGTH), null, strikethrough, compareString(d.hasMaxLength() ? toStr(d.getMaxLength()) : null, d.getMaxLengthElement(), null, "maxLength", d, compare!= null && compare.hasMaxLengthElement() ? toStr(compare.getMaxLength()) : null, null, mode, false, false)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_MIN_LENGTH), null, strikethrough, ExtensionUtilities.readStringExtension(d, ExtensionDefinitions.EXT_MIN_LENGTH)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_VALUE_REQ), null, strikethrough, compareString(encodeValue(d.getMustHaveValueElement(), null), d.getMustHaveValueElement(), null, (compare != null ? encodeValue(compare.getMustHaveValueElement(), null) : null), d, null, "mustHaveValueElement", mode, false, false));    
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_VALUE_ALT), null, strikethrough, compareSimpleTypeLists(d.getValueAlternativesList(), ((compare==null) || slicedExtension ? null : compare.getValueAlternativesList()), mode)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_DEFAULT_VALUE), null, strikethrough, encodeValue(d.getDefaultValue(), "defaultValue", d, compare==null ? null : compare.getDefaultValue(), mode, d.getName())); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_MEAN_MISS), null, strikethrough, d.getMeaningWhenMissing()); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_FIXED), null, strikethrough, encodeValue(d.getFixed(), "fixed", d, compare==null ? null : compare.getFixed(), mode, d.getName())); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_PATT_VALUE), null, strikethrough, encodeValue(d.getPattern(), "pattern", d, compare==null ? null : compare.getPattern(), mode, d.getName())); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.GENERAL_EXAMPLE), null, strikethrough, encodeValues(d.getExampleList()));
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_INVAR), null, strikethrough, invariants(d.getConstraintList(), compare==null ? null : compare.getConstraintList(), d, mode));
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOINC), null, strikethrough, getMapping(sd, d, LOINC_MAPPING, compare, mode)); 
    tableRow(tbl, context.formatPhrase(RenderingI18nContext.STRUC_DEF_SNOMED), null, strikethrough, getMapping(sd, d, SNOMED_MAPPING, compare, mode)); 
    tbl.tx("\r\n"); 
  } 
  ...
  private XhtmlNode presentModifier(ElementDefinition d, int mode, ElementDefinition compare) throws FHIRException, IOException { 
    ...
    if (x1 != null) { 
      ...
      if (x2 != null) { 
        x1.tx(" "+(context.formatPhrase(RenderingI18nContext.STRUC_DEF_BECAUSE)+" ")); 
  ...
  private XhtmlNode describeXml(StructureDefinition profile, ElementDefinition d, boolean root) { 
    ...
    for (PropertyRepresentation pr : PropertyRepresentation.values()) { 
      if (d.hasRepresentation(pr)) { 
        switch (pr) { 
        case CDATEXT: 
          ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_CDA)); 
          break; 
        case TYPEATTR: 
          ret.codeWithText((context.formatPhrase(RenderingI18nContext.STRUC_DEF_XSI)+" "), "xsi:type", "attribute."); 
          break; 
        case XHTML: 
          ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_XHTML)); 
          break; 
        case XMLATTR: 
          ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_XML_ATTRIBUTE)); 
          break; 
        case XMLTEXT: 
          ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNADORNED)); 
          break; 
        default: 
        } 
      } 
    ...
    if (name != null) { 
      ret.codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_NAMESPACE)+" ", name, "."); 
    } 
    ...
    if (name != null) { 
      ret.codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_XML_ACTUAL), name, "."); 
    } 
    return ret; 
  } 
  ...
  private XhtmlNode describeJson(ElementDefinition d) { 
    ...
    if (code != null) { 
      switch (code) { 
      case "present": 
        ul.li().tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_PRESENT)); 
        break; 
      case "absent": 
        ul.li().tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_NOT_PRESENT)); 
        break; 
      case "either": 
        ul.li().tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_MAY_PRESENT)); 
        break; 
      } 
    } 
    ...
    if (jn != null) { 
      if (d.getPath().contains(".")) { 
        ul.li().codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_PROPERTY_NAME), jn, null); 
      } else { 
        ul.li().codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_CAN_NAME), jn, " " + context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_EXT));           
      } 
    } 
    code = ExtensionUtilities.readStringExtension(d, ExtensionDefinitions.EXT_JSON_PROP_KEY); 
    if (code != null) { 
      ul.li().codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_SINGLE), code, " "+ context.formatPhrase(RenderingI18nContext.STRUC_DEF_JSON_CHILD)); 
    } 
    if (ExtensionUtilities.readBoolExtension(d, ExtensionDefinitions.EXT_JSON_NULLABLE)) { 
      ul.li().tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_NULL_JSON)); 
    } 
    if (ExtensionUtilities.readBoolExtension(d, ExtensionDefinitions.EXT_JSON_PRIMITIVE_CHOICE)) { 
      ul.li().tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_INFERRED_JSON)); 
    } 
    ...
    if (obr.hasObligations() || (root && (sdx.hasExtension(ExtensionDefinitions.EXT_OBLIGATION_PROFILE_FLAG_NEW, ExtensionDefinitions.EXT_OBLIGATION_PROFILE_FLAG_OLD) || sdx.hasExtension(ExtensionDefinitions.EXT_OBLIGATION_INHERITS_NEW, ExtensionDefinitions.EXT_OBLIGATION_INHERITS_OLD)))) {
      ...
      if (root) { 
        if (sdx.hasExtension(ExtensionDefinitions.EXT_OBLIGATION_PROFILE_FLAG_NEW, ExtensionDefinitions.EXT_OBLIGATION_PROFILE_FLAG_OLD)) {
          ul.li().tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_OBLIG_ADD));            
        }  
        for (Extension ext : sdx.getExtensionsByUrl(ExtensionDefinitions.EXT_OBLIGATION_INHERITS_NEW, ExtensionDefinitions.EXT_OBLIGATION_INHERITS_OLD)) {
          ...
          XhtmlNode bb = ul.li(); 
          bb.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_OBLIG_FROM)+" ");            
  ...
  private XhtmlNode describeAllowedUnits(ElementDefinition d) { 
    ...
    if (au instanceof CanonicalType) { 
      ...
      ret.tx(context.formatPhrase(RenderingI18nContext.GENERAL_VALUESET)+" ");          
      genCT(ret, url, vs); 
      return ret; 
    } else if (au instanceof CodeableConcept) { 
      CodeableConcept cc = (CodeableConcept) au; 
      if (cc.getCodingList().size() != 1) { 
        ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_ONE_OF)); 
      } 
      ret.tx(summarise(cc)); 
      return ret; 
    } 
  ...
  private boolean hasChoices(List<TypeRefComponent> types) { 
    for (TypeRefComponent type : types) { 
      if (type.getProfileList().size() > 1 || type.getTargetProfileList().size() > 1) { 
        return true; 
      } 
    } 
    return types.size() > 1; 
  ...
  private String sliceOrderString(ElementDefinitionSlicingComponent slicing) { 
    if (slicing.getOrdered()) 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_ORDERED); 
    else 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_UNORDERED); 
  } 
  ...
  private void generateSlicing(XhtmlNode tbl, StructureDefinition profile, ElementDefinition ed, ElementDefinitionSlicingComponent slicing, ElementDefinition compare, int mode, boolean strikethrough) throws IOException { 
    ...
    x.codeWithText(context.formatPhrase(RenderingI18nContext.STRUC_DEF_SET_SLICES)+" ", ed.getPath(), context.formatPhrase(RenderingI18nContext.STRUC_DEF_SET_ARE)); 
    ...
    x.tx(" "+context.formatPhrase(RenderingI18nContext.STRUC_DEF_AND) + " "); 
    ...
    if (slicing.hasDiscriminator()) { 
      x.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_DESCRIM)); 
  ...
  private XhtmlNode businessIdWarning(String resource, String name) { 
    if (name.equals("identifier")) { 
      XhtmlNode ret = new XhtmlNode(NodeType.Element, "div"); 
      ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_BUSINESS_ID)+" "); 
      ret.ah(context.prefixLocalHref(corePath + "resource.html#identifiers")).tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_DISCUSSION)); 
      ret.tx(")"); 
      return ret; 
    }  
    if (name.equals("version")) {// && !resource.equals("Device")) 
      XhtmlNode ret = new XhtmlNode(NodeType.Element, "div"); 
      ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_BUSINESS_VERID)+" "); 
      ret.ah(context.prefixLocalHref(corePath + "resource.html#versions")).tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_DISCUSSION)); 
      ret.tx(")"); 
      return ret; 
    } 
    return null; 
  ...
  private XhtmlNode describeCardinality(ElementDefinition d, ElementDefinition compare, int mode) throws IOException { 
    ...
    XhtmlNode t = compareSimpleTypeLists(d.getConditionList(), compare == null ? null : compare.getConditionList(), mode); 
    if (t != null) { 
      x.br(); 
      x.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_INVARIANT)+" ");  
      x.copyAllContent(t); 
    }     
    return x; 
  } 
  ...
  private XhtmlNode describeTypes(List<TypeRefComponent> types, boolean mustSupportOnly, ElementDefinition ed, ElementDefinition compare, int mode, ElementDefinition value, ElementDefinition compareValue, StructureDefinition sd) throws FHIRException, IOException { 
    ...
    List<TypeRefComponent> compareTypes = compare==null ? new ArrayList<>() : compare.getTypeList(); 
    XhtmlNode ret = new XhtmlNode(NodeType.Element, "div"); 
    if ((!mustSupportOnly && types.size() == 1 && compareTypes.size() <=1 && (mode != GEN_MODE_DIFF || !VersionComparisonAnnotation.hasDeleted(ed, "type"))) || (mustSupportOnly && mustSupportCount(types) == 1)) { 
      ...
      boolean first = true; 
      if (types.size() > 1) { 
        ret.tx(context.formatPhrase(RenderingI18nContext.STRUC_DEF_CHOICE_OF)+" "); 
      } 
  ...
  private XhtmlNode processSecondary(int mode, ElementDefinition value, ElementDefinition compareValue, int compMode, StructureDefinition sd) throws FHIRException, IOException { 
    ...
    case 2: 
      XhtmlNode x = new XhtmlNode(NodeType.Element, "div"); 
      x.tx(" "+context.formatPhrase(RenderingI18nContext.STRUC_DEF_COMP_EX)); 
      return x; 
    case 3: 
      x = new XhtmlNode(NodeType.Element, "div"); 
      x.tx(" "+context.formatPhrase(RenderingI18nContext.STRUC_DEF_EX_TYPE)+" "); 
      x.copyAllContent(describeTypes(value.getTypeList(), false, value, compareValue, compMode, null, null, sd)); 
      x.tx(")"); 
      return x; 
    default: 
      return null; 
  ...
  private void describeType(XhtmlNode x, TypeRefComponent t, boolean mustSupportOnly, TypeRefComponent compare, int mode, StructureDefinition sd) throws FHIRException, IOException { 
    ...
    if ((!mustSupportOnly && (t.hasProfile() || (compare!=null && compare.hasProfile()))) || isMustSupport(t.getProfileList())) { 
      StatusList<ResolvedCanonical> profiles = analyseProfiles(t.getProfileList(), compare == null ? null : compare.getProfileList(), mustSupportOnly, mode);
      if (profiles.size() > 0) { 
        if (!ts) { 
    ...
    if ((!mustSupportOnly && (t.hasTargetProfile() || (compare!=null && compare.hasTargetProfile()))) || isMustSupport(t.getTargetProfileList())) { 
      List<ResolvedCanonical> profiles = analyseProfiles(t.getTargetProfileList(), compare == null ? null : compare.getTargetProfileList(), mustSupportOnly, mode);       
      if (profiles.size() > 0) { 
        if (!ts) { 
          getTypeLink(unchanged(x), t, sd); 
        } 
      ...
      if (!t.getAggregationList().isEmpty() || (compare!=null && !compare.getAggregationList().isEmpty())) { 
         
        for (Enumeration<AggregationMode> a :t.getAggregationList()) { 
          a.setUserData(UserDataNames.render_link, corePath + "codesystem-resource-aggregation-mode.html#content"); 
        } 
        if (compare!=null) { 
          for (Enumeration<AggregationMode> a : compare.getAggregationList()) { 
            a.setUserData(UserDataNames.render_link, corePath + "codesystem-resource-aggregation-mode.html#content"); 
          } 
        } 
        var xt = compareSimpleTypeLists(t.getAggregationList(), compare == null ? null : compare.getAggregationList(), mode); 
        if (xt != null) { 
          x.copyAllContent(xt); 
        } 
      } 
...
//  } 
  ...
  private String conf(ElementDefinitionBindingComponent def) { 
    if (def.getStrength() == null) { 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_FOR_CODE)+" "; 
    } 
    switch (def.getStrength()) { 
    case EXAMPLE: 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_EX_CODE)+" "; 
    case PREFERRED: 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_SHOULD_CODE)+" "; 
    case EXTENSIBLE: 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_SUIT_SHALL_CODE)+" "; 
    case REQUIRED: 
      return context.formatPhrase(RenderingI18nContext.STRUC_DEF_SHALL_CODE)+" "; 
    default: 
      return "?sd-conf?"; 
    } 
  } 
  ...
  private XhtmlNode getMapping(StructureDefinition profile, ElementDefinition d, String uri, ElementDefinition compare, int mode) { 
    ...
    String oldMap = null; 
    for (ElementDefinitionMappingComponent m : compare.getMappingList()) { 
      if (m.getIdentity().equals(id)) { 
        oldMap = m.getMap(); 
        break; 
      } 
  ...
  private String summarise(CodeableConcept cc) throws FHIRException { 
    if (cc.getCodingList().size() == 1 && cc.getText() == null) { 
      ...
      throw new FHIRException(context.formatPhrase(RenderingI18nContext.STRUC_DEF_ERR_DESC)); 
    } 
  } 
 
  private String summarise(Coding coding) throws FHIRException { 
    if ("http://snomed.info/sct".equals(coding.getSystem())) 
      return "" + (context.formatPhrase(RenderingI18nContext.STRUC_DEF_SNOMED_CODE)) + " " + coding.getCode() + (!coding.hasDisplay() ? "" : "(\"" + gt(coding.getDisplayElement()) + "\")"); 
    if ("http://loinc.org".equals(coding.getSystem())) 
      return "" + (context.formatPhrase(RenderingI18nContext.STRUC_DEF_LOINC_CODE)) + " " + coding.getCode() + (!coding.hasDisplay() ? "" : "(\"" + gt(coding.getDisplayElement()) + "\")"); 
  ...
  public XhtmlNode buildElementTable(RenderingStatus status, String defFile, StructureDefinition profile, String imageFolder, boolean inlineGraphics, String profileBaseFileName, boolean snapshot, String corePath, String imagePath, 
    ...
    if (groupings == null) {
      ...
      return node;
    } else if (profile.getSnapshot().getElementList().isEmpty()) {
      XhtmlNode node = new XhtmlNode(NodeType.Element, "div");
      node.para().tx("This view is not supported for this profile because it doesn't have a snapshot");
      return node;
    } else {
      List<TableGroup> groups = new ArrayList<TableGroup>();
  ...
  private void buildElementTableRows(StructureDefinition profile, ElementTableGroupingEngine groupings, List<TableGroup> groups, List<ElementDefinition> elements) {
    for (ElementDefinition ed : elements) {
      ...
      TableGroup group = getOrMakeGroup(groupings, groups, ed);
      if (group != null) {
  ...
  private void buildElementTableRows(StructureDefinition profile, TableGroup group, List<ElementDefinition> elements, ElementDefinition parent) {
    for (ElementDefinition ed : elements) {
      if (hasAnyDiff(profile, ed) && !ed.isProhibited()) {
        boolean isLeaf = isLeaf(ed);
        if (isLeaf) {
  ...
  private TableElement makeElement(StructureDefinition profile, ElementDefinition parentDefn, ElementDefinition snapDefn, boolean lookAtChildren) {
    ...
    if (snapDefn.getTypeList().size() > 1) {
      List<String> list = new ArrayList<String>();
      for (TypeRefComponent tr : snapDefn.getTypeList()) {
      ...
      e.getConstraints().add(TableElementConstraint.makeTypes(TableElementConstraintType.CHOICE, null, snapDefn.getTypeList(), profile));
    } else {
      ...
      if (snapDefn.getTypeFirstRep().hasProfile()) {
        e.getConstraints().add(TableElementConstraint.makeList(TableElementConstraintType.PROFILE, null, snapDefn.getTypeFirstRep().getProfileList(), profile));
      }
      if (snapDefn.getTypeFirstRep().hasTargetProfile()) {
        e.getConstraints().add(TableElementConstraint.makeList(TableElementConstraintType.TARGET, null, snapDefn.getTypeFirstRep().getTargetProfileList(), profile));
      }
    }
  ...
  private String chooseIcon(StructureDefinition profile, ElementDefinition element, TypeRefComponent tr) {
    if (tail(element.getPath()).equals("extension") && isExtension(element)) { 
      if (element.hasType() && element.getTypeList().get(0).hasProfile() && extensionIsComplex(element.getTypeList().get(0).getProfileList().get(0).getValue(), profile))
    ...
    } else if (element.getTypeList().size() == 0) { 
    ...
    } else if (element.getTypeList().size() > 1) { 
      if (allAreReference(element.getTypeList())) { 
    ...
    } else if (element.getTypeList().get(0).getWorkingCode() != null && element.getTypeList().get(0).getWorkingCode().startsWith("@")) { 
      return "icon_reuse.png"; 
    } else if (context.getContext().isPrimitiveType(element.getTypeList().get(0).getWorkingCode())) { 
      if (keyRows.contains(element.getId())) { 
        return "icon-key.png"; 
      } else { 
        return "icon_primitive.png"; 
      } 
    } else if (element.getTypeList().get(0).hasTarget()) { 
      return "icon_reference.png"; 
    } else if (Utilities.existsInList(element.getTypeList().get(0).getWorkingCode(), "Element", "BackboneElement")) { 
      return "icon_group.png"; 
    } else if (Utilities.existsInList(element.getTypeList().get(0).getWorkingCode(), "Base", "Element", "BackboneElement")) { 
      return "icon_element.gif"; 
    } else if (context.getContext().isDataType(element.getTypeList().get(0).getWorkingCode())) { 
  ...
  public XhtmlNode compositionSummary(StructureDefinition sd) throws FHIRException, IOException, EOperationOutcome {
    ...
    XhtmlNode x = new XhtmlNode(NodeType.Element, "div");
    x.para().tx(context.formatPhrase(RenderingI18nContext.STRUCTURAL_REQUIREMENTS));
  ...
  private StructureDefinition findMatchingEntryProfile(ElementWithInvariant inv, String criteria) {
    ...
    if (sd == null) {
      sd = context.getContext().fetchResource(StructureDefinition.class, criteria, VersionResolutionRules.PACKAGE);
    }
    if (sd != null) {
      return sd;
    }
  ...
  private String getCodeForSection(StructureDefinition sd, ElementDefinition edSection) {
    int index = sd.getSnapshot().getElementList().indexOf(edSection)+1;
    while (index < sd.getSnapshot().getElementList().size()) {
      ElementDefinition ed = sd.getSnapshot().getElementList().get(index);
      if (isSectionElement(ed)) {
        return null;
      }
