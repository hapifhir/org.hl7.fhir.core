...
public abstract class CanonicalResourceComparer extends ResourceComparer {
  ...
  protected boolean compareMetadata(CanonicalResource left, CanonicalResource right, Map<String, StructuralMatch<String>> comp, CanonicalResourceComparison<? extends CanonicalResource> res, List<String> changes, Base parent) {
    ...
    if (compareCodeableConceptList("jurisdiction", left.getJurisdictionList(), right.getJurisdictionList(), comp, IssueSeverity.INFORMATION, res, res.getUnion().getJurisdictionList(), res.getIntersection().getJurisdictionList())) {
      changed = true;
  ...
  protected <T extends Enum<?>> void compareEnumList(String name, List<Enumeration<T>> left, List<Enumeration<T>> right, Map<String, StructuralMatch<String>> comp, IssueSeverity level, CanonicalResourceComparison<? extends CanonicalResource> res, List<Enumeration<T>> union, List<Enumeration<T>> intersection ) {
    List<Enumeration<T>> matchR = new ArrayList<>();
    StructuralMatch<String> combined = new StructuralMatch<String>();
    for (Enumeration<T> l : left) {
      Enumeration<T> r = findEnumInList(right, l);
      if (r == null) {
        union.add(l);
        combined.getChildren().add(new StructuralMatch<String>(l.getCode(), vm(IssueSeverity.INFORMATION, "Removed the item '"+l.getValue()+"'", fhirType()+"."+name, res.getMessages())));
      } else {
        matchR.add(r);
        union.add(r);
        intersection.add(r);
        StructuralMatch<String> sm = new StructuralMatch<String>(l.getCode(), r.getCode());
        combined.getChildren().add(sm);
      }
    }
    for (Enumeration<T> r : right) {
      if (!matchR.contains(r)) {
        union.add(r);
        combined.getChildren().add(new StructuralMatch<String>(vm(IssueSeverity.INFORMATION, "Added the item '"+r.getCode()+"'", fhirType()+"."+name, res.getMessages()), r.getCode()));
      }
    }
    comp.put(name, combined);
  }
  ...
  private <T extends Enum<?>> Enumeration<T> findEnumInList(List<Enumeration<T>> list, Enumeration<T> item) {
    for (Enumeration<T> t : list) {
