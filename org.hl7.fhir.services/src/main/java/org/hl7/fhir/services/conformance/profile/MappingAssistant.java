...
public class MappingAssistant {
  ...
  public MappingAssistant(MappingMergeModeOption mappingMergeMode, StructureDefinition base, StructureDefinition derived, String version, List<String> suppressedMappings) {
    ...
    for (StructureDefinitionMappingComponent m : derived.getMappingList()) {
      masterList.add(m);
      if (!isSuppressed(m)) {
    ...
    for (StructureDefinitionMappingComponent m : base.getMappingList()) {
      if (notExcluded(m)) {
        ...
        if (md == null) {
          if (nameExists(m.getIdentity())) {
            ...
            masterList.add(m.copy(Base.COPY_DATA).setIdentity(n));
          } else {
            masterList.add(m.copy(Base.COPY_DATA));
          }
        } else {
  ...
  private boolean notExcluded(ElementDefinitionMappingComponent m) {
    ...
    for (StructureDefinitionMappingComponent t : base.getMappingList()) {
  ...
  private StructureDefinitionMappingComponent findMatchInDerived(StructureDefinitionMappingComponent m) {
    for (StructureDefinitionMappingComponent md : derived.getMappingList()) {
  ...
  public void merge(ElementDefinition base, ElementDefinition derived) {
    ...
    addMappings(list, base.getMappingList(), renames);
    if (derived.hasMapping()) {
      addMappings(list, derived.getMappingList(), null);
    }
    derived.setMappingList(list);
    
    // trim anything
    for (ElementDefinitionMappingComponent m : base.getMappingList()) {
      if (m.hasMap()) {
  ...
  private boolean isSuppressed(Element s) {
    return ExtensionUtilities.readBoolExtension(s, ExtensionDefinitions.EXT_ED_SUPPRESS);
  }
