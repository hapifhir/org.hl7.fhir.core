...
//    compareExpansions(left, right, res);
  ...
  private boolean compareCompose(ValueSet.ValueSetComposeComponent left, ValueSet.ValueSetComposeComponent right, ValueSetComparison res, ValueSet.ValueSetComposeComponent union, ValueSet.ValueSetComposeComponent intersection) {
    boolean def = false;
    // first, the includes
    List<ValueSet.ConceptSetComponent> matchR = new ArrayList<>();
    for (ValueSet.ConceptSetComponent l : left.getIncludeList()) {
      ValueSet.ConceptSetComponent r = findInList(right.getIncludeList(), l, left.getIncludeList());
      if (r == null) {
        union.getIncludeList().add(l);
        ...
        matchR.add(r);
        ValueSet.ConceptSetComponent csM = new ValueSet.ConceptSetComponent();
        ValueSet.ConceptSetComponent csI = new ValueSet.ConceptSetComponent();
        union.getIncludeList().add(csM);
        intersection.getIncludeList().add(csI);
        StructuralMatch<Element> sm = new StructuralMatch<Element>(l, r);
        res.getIncludes().getChildren().add(sm);
        def = compareDefinitions("ValueSet.compose.include["+right.getIncludeList().indexOf(r)+"]", l, r, sm, csM, csI, res) || def;
      }
    }
    for (ValueSet.ConceptSetComponent r : right.getIncludeList()) {
      if (!matchR.contains(r)) {
        union.getIncludeList().add(r);
        res.updateContentState(true);
    ...
    // now. the excludes
    matchR.clear();
    for (ValueSet.ConceptSetComponent l : left.getExcludeList()) {
      ValueSet.ConceptSetComponent r = findInList(right.getExcludeList(), l, left.getExcludeList());
      if (r == null) {
        union.getExcludeList().add(l);
        res.updateContentState(true);
        ...
        ValueSet.ConceptSetComponent csM = new ValueSet.ConceptSetComponent();
        ValueSet.ConceptSetComponent csI = new ValueSet.ConceptSetComponent();
        union.getExcludeList().add(csM);
        intersection.getExcludeList().add(csI);
        StructuralMatch<Element> sm = new StructuralMatch<Element>(l, r);
        res.getExcludes().getChildren().add(sm);
        def = compareDefinitions("ValueSet.compose.exclude["+right.getExcludeList().indexOf(r)+"]", l, r, sm, csM, csI, res) || def;
      }
    }
    for (ValueSet.ConceptSetComponent r : right.getExcludeList()) {
      if (!matchR.contains(r)) {
        union.getExcludeList().add(r);
        res.updateContentState(true);
  ...
  private boolean compareDefinitions(String path, ValueSet.ConceptSetComponent left, ValueSet.ConceptSetComponent right, StructuralMatch<Element> combined, ValueSet.ConceptSetComponent union, ValueSet.ConceptSetComponent intersection, ValueSetComparison res) {
    ...
    for (CanonicalType l : left.getValueSetList()) {
      CanonicalType r = findInList(right.getValueSetList(), l, left.getValueSetList());
      if (r == null) {
        union.getValueSetList().add(l);
        res.updateContentState(true);
    ...
    for (CanonicalType r : right.getValueSetList()) {
    ...
    List<ValueSet.ConceptReferenceComponent> matchCR = new ArrayList<>();
    for (ValueSet.ConceptReferenceComponent l : left.getConceptList()) {
      ValueSet.ConceptReferenceComponent r = findInList(right.getConceptList(), l, left.getConceptList());
      if (r == null) {
        union.getConceptList().add(l);
        res.updateContentState(true);
        ...
        if (l.getCode().equals(r.getCode())) {
          ...
          union.getConceptList().add(cu);
          intersection.getConceptList().add(ci);
          StructuralMatch<Element> sm = new StructuralMatch<Element>(l, r);
          combined.getChildren().add(sm);
          def = compareConcepts(path+".concept["+right.getConceptList().indexOf(r)+"]", l, r, sm, cu, ci, res) || def;
        } else {
          // not that it's possible to get here?
          union.getConceptList().add(l);
          union.getConceptList().add(r);
          ...
          combined.getChildren().add(sm);
          res.updateContentState(true);
          compareConcepts(path+".concept["+right.getConceptList().indexOf(r)+"]", l, r, sm, null, null, res);
          session.markChanged(r, l);
        }
      }
    }
    for (ValueSet.ConceptReferenceComponent r : right.getConceptList()) {
    ...
    List<ValueSet.ConceptSetFilterComponent> matchFR = new ArrayList<>();
    for (ValueSet.ConceptSetFilterComponent l : left.getFilterList()) {
      ValueSet.ConceptSetFilterComponent r = findInList(right.getFilterList(), l, left.getFilterList());
      if (r == null) {
        union.getFilterList().add(l);
        res.updateContentState(true);
        ...
        if (l.getProperty().equals(r.getProperty()) && l.getOp().equals(r.getOp())) {
          ...
          union.getFilterList().add(cu);
          intersection.getFilterList().add(ci);
          StructuralMatch<Element> sm = new StructuralMatch<Element>(l, r);
          combined.getChildren().add(sm);
    ...
    for (ValueSet.ConceptSetFilterComponent r : right.getFilterList()) {
  ...
  private void compareExpansions(ValueSet left, ValueSet right, ValueSetComparison res) {
    ...
    if (expL != null && expR != null) {
      ...
      compareConcepts(expL.getExpansion().getContainsList(), expR.getExpansion().getContainsList(), res.forceExpansion(), res.getUnion().getExpansion().getContainsList(), res.getIntersection().getExpansion().getContainsList(), "ValueSet.expansion.contains", res);
    }
  }
  ...
  private void compareConcepts(List<ValueSet.ValueSetExpansionContainsComponent> left, List<ValueSet.ValueSetExpansionContainsComponent> right, StructuralMatch<ValueSet.ValueSetExpansionContainsComponent> combined, List<ValueSet.ValueSetExpansionContainsComponent> union, List<ValueSet.ValueSetExpansionContainsComponent> intersection, String path, ValueSetComparison res) {
    List<ValueSet.ValueSetExpansionContainsComponent> matchR = new ArrayList<>();
    for (ValueSet.ValueSetExpansionContainsComponent l : left) {
      ValueSet.ValueSetExpansionContainsComponent r = findInList(right, l);
      if (r == null) {
        union.add(l);
        combined.getChildren().add(new StructuralMatch<ValueSet.ValueSetExpansionContainsComponent>(l, vmI(IssueSeverity.INFORMATION, "Removed from expansion", path)));
      } else {
        matchR.add(r);
        ValueSet.ValueSetExpansionContainsComponent ccU = merge(l, r);
        ValueSet.ValueSetExpansionContainsComponent ccI = intersect(l, r);
        union.add(ccU);
        ...
        combined.getChildren().add(sm);
        compareConcepts(l.getContainsList(), r.getContainsList(), sm, ccU.getContainsList(), ccI.getContainsList(), path+".where(code = '"+l.getCode()+"').contains", res);
      }
    }
    for (ValueSet.ValueSetExpansionContainsComponent r : right) {
      if (!matchR.contains(r)) {
        union.add(r);
        combined.getChildren().add(new StructuralMatch<ValueSet.ValueSetExpansionContainsComponent>(vmI(IssueSeverity.INFORMATION, "Added to expansion", path), r));        
      }
    }
  }
  ...
  private void addComposeRow(HierarchicalTableGenerator gen, List<Row> rows, StructuralMatch<Element> t, String name) {
    ...
    if (t.hasLeft() && t.hasRight()) {
      ...
      ValueSet.ConceptSetComponent cs = (ValueSet.ConceptSetComponent) t.getLeft();
      ...
      ValueSet.ConceptSetComponent cs = (ValueSet.ConceptSetComponent) t.getRight();
      r.getCells().add(missingCell(gen));
      r.getCells().add(gen.new Cell(null, null, cs.getSystem(), null, null));
      r.getCells().add(missingCell(gen));
      r.getCells().add(gen.new Cell(null, null, cs.hasVersion() ? "Version: "+cs.getVersion() : "", null, null));
...
//    }
  ...
  private void addExpansionRow(HierarchicalTableGenerator gen, List<Row> rows, StructuralMatch<ValueSet.ValueSetExpansionContainsComponent> t, boolean hasSystem, boolean hasVersion, boolean hasAbstract, boolean hasInactive) {
    Row r = gen.new Row();
    rows.add(r);
    if (hasSystem) {
  ...
  public XhtmlNode renderUnion(ValueSetComparison comp, String id, String prefix, String corePath) throws FHIRFormatError, DefinitionException, FHIRException, IOException, EOperationOutcome {
    ValueSetRenderer vsr = new ValueSetRenderer(new RenderingContext(session.getContextLeft(), new RendererFactory(), null, new ValidationOptions(), corePath, prefix, session.getContextLeft().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER));
