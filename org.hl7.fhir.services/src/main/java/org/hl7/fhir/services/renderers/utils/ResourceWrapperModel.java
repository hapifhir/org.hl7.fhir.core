package org.hl7.fhir.services.renderers.utils;
...
public class ResourceWrapperModel extends ResourceWrapper {
  ...
  protected void loadTheChildren() {
    for (Element child : model.getChildList()) {
  ...
  public void setNarrative(XhtmlNode x, String status, boolean multiLangMode, Locale locale, boolean isPretty) throws IOException {
    org.hl7.fhir.services.elementmodel.Element txt = model.getNamedChild("text");
    if (txt == null) {
    ...
    org.hl7.fhir.services.elementmodel.Element st = txt.getNamedChild("status");
    if (st == null) {
      st = new org.hl7.fhir.services.elementmodel.Element("status", txt.getProperty().getChild(null, "status"));
      txt.getChildList().add(0, st);
    }
    st.setValue(status);
    org.hl7.fhir.services.elementmodel.Element div = txt.getNamedChild("div");
    if (div == null) {
      div = new org.hl7.fhir.services.elementmodel.Element("div", txt.getProperty().getChild(null, "div"));
      txt.getChildList().add(div);
    } 
  ...
  public String getCodeSystemUri() {
    ...
    if (pd != null && pd.hasBinding() && pd.getBinding().hasValueSet()) { 
      ...
      if (vs != null && vs.hasCompose() && !vs.getCompose().hasExclude() && vs.getCompose().getIncludeList().size() == 1) {
