...
public class ComparisonSession {
  ...
  public ResourceComparer.ResourceComparison compare(String left, Resource leftSource, String right, Resource rightSource) throws DefinitionException, FHIRFormatError, IOException {
    CanonicalResource l = (CanonicalResource) contextLeft.fetchResource(Resource.class, left, VersionResolutionRules.defaultRule(), null, leftSource);
    if (l == null) {
      throw new DefinitionException("Unable to resolve "+left);
    }
    CanonicalResource r = (CanonicalResource) contextRight.fetchResource(Resource.class, right, VersionResolutionRules.defaultRule(), null, rightSource);
    if (r == null) {
