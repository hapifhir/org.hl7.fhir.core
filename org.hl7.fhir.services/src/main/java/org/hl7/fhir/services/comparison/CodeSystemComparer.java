...
import org.hl7.fhir.services.comparison.CanonicalResourceComparer;
import org.hl7.fhir.services.comparison.ComparisonSession;
import org.hl7.fhir.services.comparison.StructuralMatch;
...
public class CodeSystemComparer extends CanonicalResourceComparer {
  ...
  public CodeSystemComparison compare(CodeSystem left, CodeSystem right) {
    ...
    ch = compareProperties(left.getPropertyList(), right.getPropertyList(), res.getProperties(), res.getUnion().getPropertyList(), res.getIntersection().getPropertyList(), res.getUnion(), res.getIntersection(), res, "CodeSystem.property", right) || ch;
    ch = compareFilters(left.getFilterList(), right.getFilterList(), res.getFilters(), res.getUnion().getFilterList(), res.getIntersection().getFilterList(), res.getUnion(), res.getIntersection(), res, "CodeSystem.filter", right) || ch;
    ch = compareConcepts(left.getConceptList(), right.getConceptList(), res.getCombined(), res.getUnion().getConceptList(), res.getIntersection().getConceptList(), res.getUnion(), res.getIntersection(), res, "CodeSystem.concept", right) || ch;
  ...
  private PropertyComponent findProperty(CodeSystem left, PropertyComponent p) {
    for (PropertyComponent t : left.getPropertyList()) {
  ...
  private boolean compareConcepts(List<ConceptDefinitionComponent> left, List<ConceptDefinitionComponent> right, StructuralMatch<ConceptDefinitionComponent> combined,
    ...
    for (ConceptDefinitionComponent l : left) {
      ...
      if (r == null) {
        ...
        matchR.add(r);
        ConceptDefinitionComponent cdM = merge(l, r, csU.getPropertyList(), res);
        ...
        combined.getChildren().add(sm);
        if (compareConcepts(l.getConceptList(), r.getConceptList(), sm, cdM.getConceptList(), cdI.getConceptList(), csU, csI, res, path+".where(code='"+l.getCode()+"').concept", r)) {
          def = true;
        }
      }
    }
...
//    result = compareStrings(path, msgs, l.hasOperator() ? l.getOperator().toCode() : null, r.hasType() ? r.getType().toCode() : null, "type", IssueSeverity.ERROR, res, parent, l.getTypeElement(), r.getTypeElement()) || result;
  ...
  private void mergeDesignations(ConceptDefinitionComponent cd, ConceptDefinitionComponent l, ConceptDefinitionComponent r) {
    for (ConceptDefinitionDesignationComponent td : l.getDesignationList()) {
      if (hasDesignation(td, r.getDesignationList())) {
        cd.getDesignationList().add(td);
      }
    }
    for (ConceptDefinitionDesignationComponent td : r.getDesignationList()) {
      if (hasDesignation(td, l.getDesignationList())) {
        cd.getDesignationList().add(td);
      }
    }
  }
  ...
  private void mergeProps(ConceptDefinitionComponent cd, ConceptDefinitionComponent l, ConceptDefinitionComponent r, List<PropertyComponent> destProps, CodeSystemComparison res) {
    ...
    for (ConceptPropertyComponent lp : l.getPropertyList()) {
      ConceptPropertyComponent rp = findRightProp(r.getPropertyList(), lp, res);
      if (rp == null) {
  ...
  public XhtmlNode renderConcepts(CodeSystemComparison comparison, String id, String prefix) throws FHIRException, IOException {
    ...
    for (PropertyComponent p : comparison.getUnion().getPropertyList()) {
  ...
  public ConceptPropertyComponent getProp(ConceptDefinitionComponent cd, PropertyComponent p, boolean right, CodeSystemComparison comp) {
    ...
    if (cd != null) {
      for (ConceptPropertyComponent t : cd.getPropertyList()) {
  ...
  public XhtmlNode renderUnion(CodeSystemComparison comp, String id, String prefix, String corePath) throws FHIRFormatError, DefinitionException, FHIRException, IOException, EOperationOutcome {
    CodeSystemRenderer csr = new CodeSystemRenderer(new RenderingContext(session.getContextLeft(), new RendererFactory(), null, new ValidationOptions(), corePath, prefix, session.getContextLeft().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER));
    return csr.buildNarrative(ResourceWrapper.forResource(csr.getContext(), comp.getUnion()));
  }
  ...
  public XhtmlNode renderIntersection(CodeSystemComparison comp, String id, String prefix, String corePath) throws FHIRFormatError, DefinitionException, FHIRException, IOException, EOperationOutcome {
    CodeSystemRenderer csr = new CodeSystemRenderer(new RenderingContext(session.getContextLeft(), new RendererFactory(), null, new ValidationOptions(), corePath, prefix, session.getContextLeft().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER));
    return csr.buildNarrative(ResourceWrapper.forResource(csr.getContext(), comp.getIntersection()));
  }
