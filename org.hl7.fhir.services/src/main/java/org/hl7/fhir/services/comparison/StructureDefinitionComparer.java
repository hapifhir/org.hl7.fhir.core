...
//    ruleEqual(comp, res, left.current().getIsSummaryElement(), right.current().getIsSummaryElement(), "isSummary", path); - so does this
    ...
    subset.getRepresentationList().addAll(left.current().getRepresentationList()); // can't be bothered even testing this one
    ...
    subset.getCodeList().addAll(mergeCodings(left.current().getCodeList(), right.current().getCodeList()));
    subset.getAliasList().addAll(mergeStrings(left.current().getAliasList(), right.current().getAliasList()));
    subset.getMappingList().addAll(mergeMappings(left.current().getMappingList(), right.current().getMappingList()));
    // left will win for example
    subset.setExampleList(left.current().hasExample() ? left.current().getExampleList() : right.current().getExampleList());
    ...
    superset.getTypeList().addAll(unionTypes(comp, res, path, left.current().getTypeList(), right.current().getTypeList(), left.getStructure(), right.getStructure()));
    subset.getTypeList().addAll(intersectTypes(comp, res, subset, path, left.current().getTypeList(), right.current().getTypeList()));
    rule(comp, res, !subset.getTypeList().isEmpty() || (!left.current().hasType() && !right.current().hasType()), path, "Type Mismatch: "+typeCode(left)+" vs "+typeCode(right));
    ...
    superset.getConstraintList().addAll(intersectConstraints(path, left.current().getConstraintList(), right.current().getConstraintList()));
    subset.getConstraintList().addAll(unionConstraints(comp, res, path, left.current().getConstraintList(), right.current().getConstraintList()));
    comp.getIntersection().getSnapshot().getElementList().add(subset);
    comp.getUnion().getSnapshot().getElementList().add(superset);
...
//    return null;
  ...
  private boolean compareChildren(ProfileComparison comp, StructuralMatch<ElementDefinitionNode> res, String path, DefinitionNavigator left, DefinitionNavigator right) throws DefinitionException, IOException, FHIRFormatError {
    ...
    // walk into it
    if (lc.isEmpty() && !rc.isEmpty() && right.current().getTypeList().size() == 1 && left.hasTypeChildren(right.current().getTypeList().get(0), left.getStructure()))
      lc = left.childrenFromType(right.current().getTypeList().get(0), right.getStructure());
    if (rc.isEmpty() && !lc.isEmpty() && left.current().getTypeList().size() == 1 && right.hasTypeChildren(left.current().getTypeList().get(0), right.getStructure()))
      rc = right.childrenFromType(left.current().getTypeList().get(0), left.getStructure());
...
//    ruleEqual(comp, res, left.current().getIsSummaryElement(), right.current().getIsSummaryElement(), "isSummary", path); - so does this
    ...
    if (edl == null && edr == null) {
      ...
      comparePrimitivesWithTracking("alias", edl.getAliasList(), edr.getAliasList(), null, IssueSeverity.INFORMATION, null, edr);
      compareDataTypesWithTracking("code", edl.getCodeList(), edr.getCodeList(), null, IssueSeverity.INFORMATION, null, edr);
      ...
      ElementDefinition.ElementDefinitionBindingComponent bl = edl.getBinding();
      ElementDefinition.ElementDefinitionBindingComponent br = edr.getBinding();
      if (bl == null && br == null) {
        // both are sparse at this point, do nothing
      } else if (bl == null) {
        session.markAdded(edr);      
...
//    return null;
  ...
  private boolean compareTypes(String path, String sliceName, ElementDefinition left, ElementDefinition right, ProfileComparison res) {
    ...
    List<ElementDefinition.TypeRefComponent> matchR = new ArrayList<>();
    for (ElementDefinition.TypeRefComponent l : left.getTypeList()) {
      ElementDefinition.TypeRefComponent r = findInList(right.getTypeList(), l);
      if (r == null) {
    ...
    for (ElementDefinition.TypeRefComponent r : right.getTypeList()) {
      if (!matchR.contains(r)) {
        session.markAdded(r);
        res.updateContentState(true);
      }
  ...
  private ElementDefinition.TypeRefComponent findInList(List<ElementDefinition.TypeRefComponent> rc, ElementDefinition.TypeRefComponent l) {
    for (ElementDefinition.TypeRefComponent t : rc) {
  ...
  private boolean compareType(String string, ElementDefinition.TypeRefComponent l, ElementDefinition.TypeRefComponent r, ProfileComparison res) {
    boolean def = false;
    boolean ch = false;
    // codes must match
    ch = comparePrimitivesWithTracking("profile", l.getProfileList(), r.getProfileList(), null, IssueSeverity.ERROR, null, r) || ch;
    ch = comparePrimitivesWithTracking("targetProfile", l.getTargetProfileList(), r.getTargetProfileList(), null, IssueSeverity.ERROR, null, r) || ch;
    ch = comparePrimitivesWithTracking("aggregation", l.getAggregationList(), r.getAggregationList(), null, IssueSeverity.ERROR, null, r) || ch;
  ...
  private boolean compareInvariants(String path, String sliceName, ElementDefinition left, ElementDefinition right, ProfileComparison res) {
    ...
    List<ElementDefinition.ElementDefinitionConstraintComponent> matchR = new ArrayList<>();
    for (ElementDefinition.ElementDefinitionConstraintComponent l : left.getConstraintList()) {
      ElementDefinition.ElementDefinitionConstraintComponent r = findInList(right.getConstraintList(), l);
      if (r == null) {
    ...
    for (ElementDefinition.ElementDefinitionConstraintComponent r : right.getConstraintList()) {
      if (!matchR.contains(r)) {
        session.markAdded(r);
        res.updateContentState(true);
      }
  ...
  private ElementDefinition.ElementDefinitionConstraintComponent findInList(List<ElementDefinition.ElementDefinitionConstraintComponent> rc, ElementDefinition.ElementDefinitionConstraintComponent l) {
    for (ElementDefinition.ElementDefinitionConstraintComponent t : rc) {
  ...
  private boolean compareInvariant(String string, ElementDefinition.ElementDefinitionConstraintComponent l, ElementDefinition.ElementDefinitionConstraintComponent r, ProfileComparison res) {
    boolean def = false;
    boolean ch = false;
    // codes must match
...
//
  ...
  private List<ElementDefinition.ElementDefinitionMappingComponent> mergeMappings(List<ElementDefinition.ElementDefinitionMappingComponent> left, List<ElementDefinition.ElementDefinitionMappingComponent> right) {
    List<ElementDefinition.ElementDefinitionMappingComponent> result = new ArrayList<ElementDefinition.ElementDefinitionMappingComponent>();
    result.addAll(left);
...
//    rule(comp, res, subset.getMax().equals("*") || Integer.parseInt(subset.getMax()) >= subset.getMin(), path, "Cardinality Mismatch: "+card(left)+"/"+card(right));
  ...
  private Collection<? extends ElementDefinition.TypeRefComponent> unionTypes(ProfileComparison comp, StructuralMatch<ElementDefinitionNode> res, String path, List<ElementDefinition.TypeRefComponent> left, List<ElementDefinition.TypeRefComponent> right, Resource leftSrc, Resource rightSrc) throws DefinitionException, IOException, FHIRFormatError {
    List<ElementDefinition.TypeRefComponent> result = new ArrayList<ElementDefinition.TypeRefComponent>();
    ...
    for (ElementDefinition.TypeRefComponent ex : results) {
      if (Utilities.equals(ex.getWorkingCode(), nw.getWorkingCode())) {
        ...
        if (!ex.hasProfile() && !nw.hasProfile())
          ...
          pfound = true;
          ex.setProfileList(null);
        } else {
          // both have profiles. Is one derived from the other? 
          StructureDefinition sdex = ((IWorkerContext) ex.getUserData(UserDataNames.COMP_CONTEXT)).fetchResource(StructureDefinition.class, ex.getProfileList().get(0).getValue(),
            ExtensionUtilities.getVersionResolutionRules(ex.getProfileList().get(0)), null, nwSource);
          StructureDefinition sdnw = ctxt.fetchResource(StructureDefinition.class, nw.getProfileList().get(0).getValue(),
            ExtensionUtilities.getVersionResolutionRules(nw.getProfileList().get(0)), null, nwSource);
          if (sdex != null && sdnw != null) {
            if (sdex.getUrl().equals(sdnw.getUrl())) {
              ...
              ex.setProfileList(nw.getProfileList());
              pfound = true;
            } else if (derivesFrom(sdnw, sdex, ctxt)) {
              pfound = true;
            } else if (sdnw.getSnapshot().getElementList().get(0).getPath().equals(sdex.getSnapshot().getElementList().get(0).getPath())) {
        ...
        if (!ex.hasTargetProfile() && !nw.hasTargetProfile())
          ...
          StructureDefinition sdex = ((IWorkerContext) ex.getUserData(UserDataNames.COMP_CONTEXT)).fetchResource(StructureDefinition.class,
            ex.getTargetProfileList().get(0).getValue(), ExtensionUtilities.getVersionResolutionRules(ex.getTargetProfileList().get(0)), null, nwSource);
          StructureDefinition sdnw = ctxt.fetchResource(StructureDefinition.class,
            nw.getTargetProfileList().get(0).getValue(), ExtensionUtilities.getVersionResolutionRules(nw.getTargetProfileList().get(0)), null, nwSource);
          if (sdex != null && sdnw != null) {
            if (matches(sdex, sdnw)) {
              tfound = true;
            } else if (derivesFrom(sdex, sdnw, ((IWorkerContext) ex.getUserData(UserDataNames.COMP_CONTEXT)))) {
              ex.setTargetProfileList(nw.getTargetProfileList());
              tfound = true;
            } else if (derivesFrom(sdnw, sdex, ctxt)) {
              tfound = true;
            } else if (sdnw.getSnapshot().getElementList().get(0).getPath().equals(sdex.getSnapshot().getElementList().get(0).getPath())) {
  ...
  private Collection<? extends ElementDefinition.TypeRefComponent> intersectTypes(ProfileComparison comp, StructuralMatch<ElementDefinitionNode> res, ElementDefinition ed, String path, List<ElementDefinition.TypeRefComponent> left, List<ElementDefinition.TypeRefComponent> right) throws DefinitionException, IOException, FHIRFormatError {
    List<ElementDefinition.TypeRefComponent> result = new ArrayList<ElementDefinition.TypeRefComponent>();
    for (ElementDefinition.TypeRefComponent l : left) {
      boolean pfound = false;
      boolean tfound = false;
      ...
      for (ElementDefinition.TypeRefComponent r : right) {
        if (!l.hasProfile() && !r.hasProfile()) {
          ...
          pfound = true;
          c.setProfileList(r.getProfileList());
        } else {
          StructureDefinition sdl = resolveProfile(comp, res, path, l.getProfileList().get(0), comp.getLeft().getName(), session.getContextLeft(), comp.getLeft());
          StructureDefinition sdr = resolveProfile(comp, res, path, r.getProfileList().get(0), comp.getRight().getName(), session.getContextRight(), comp.getRight());
        ...
        if (!l.hasTargetProfile() && !r.hasTargetProfile()) {
          ...
          tfound = true;
          c.setTargetProfileList(r.getTargetProfileList());
        } else {
          StructureDefinition sdl = resolveProfile(comp, res, path, l.getTargetProfileList().get(0), comp.getLeft().getName(), session.getContextLeft(), comp.getLeft());
          StructureDefinition sdr = resolveProfile(comp, res, path, r.getTargetProfileList().get(0), comp.getRight().getName(), session.getContextRight(), comp.getRight());
          if (sdl != null && sdr != null) {
            if (matches(sdl, sdr)) {
              tfound = true;
            } else if (derivesFrom(sdl, sdr, session.getContextLeft())) {
        ...
        if (pfound && tfound) {
          for (Enumeration<ElementDefinition.AggregationMode> a : l.getAggregationList()) {
            if (!r.hasAggregation(a.getValue())) {
              c.getAggregationList().removeIf(n -> n.getValue() == a.getValue());
            }
          }
        }
      }
  ...
  private String typeCode(DefinitionNavigator defn) {
    ...
    for (ElementDefinition.TypeRefComponent t : defn.current().getTypeList())
      b.append(t.getWorkingCode()+(t.hasProfile() ? "("+t.getProfileList()+")" : "")+(t.hasTargetProfile() ? "("+t.getTargetProfileList()+")" : "")); // todo: other properties
    return b.toString();
  }
  ...
  private boolean compareBindings(ProfileComparison comp, StructuralMatch<ElementDefinitionNode> res, ElementDefinition subset, ElementDefinition superset, String path, ElementDefinition lDef, ElementDefinition rDef, Resource leftSrc, Resource rightSrc) throws FHIRFormatError, DefinitionException, IOException {
    ...
    ElementDefinition.ElementDefinitionBindingComponent right = rDef.getBinding();
    ...
    if (Base.compareDeep(left.getValueSet(), right.getValueSet(), false)) {
      ...
      if (lvs == null) {
        ...
        ValueSetComparer.ValueSetComparison compP = (ValueSetComparer.ValueSetComparison) session.compare(lvs, rvs);
        if (compP != null) {
  ...
  private List<ElementDefinition.ElementDefinitionConstraintComponent> intersectConstraints(String path, List<ElementDefinition.ElementDefinitionConstraintComponent> left, List<ElementDefinition.ElementDefinitionConstraintComponent> right) {
    List<ElementDefinition.ElementDefinitionConstraintComponent> result = new ArrayList<ElementDefinition.ElementDefinitionConstraintComponent>();
  ...
  private List<ElementDefinition.ElementDefinitionConstraintComponent> unionConstraints(ProfileComparison comp, StructuralMatch<ElementDefinitionNode> res, String path, List<ElementDefinition.ElementDefinitionConstraintComponent> left, List<ElementDefinition.ElementDefinitionConstraintComponent> right) {
    List<ElementDefinition.ElementDefinitionConstraintComponent> result = new ArrayList<ElementDefinition.ElementDefinitionConstraintComponent>();
    for (ElementDefinition.ElementDefinitionConstraintComponent l : left) {
      boolean found = false;
      for (ElementDefinition.ElementDefinitionConstraintComponent r : right)
  ...
  private ElementDefinition.ElementDefinitionBindingComponent unionBindings(ProfileComparison comp, StructuralMatch<ElementDefinitionNode> res, String path, ElementDefinition.ElementDefinitionBindingComponent left, ElementDefinition.ElementDefinitionBindingComponent right, Resource leftSrc, Resource rightSrc) throws FHIRFormatError, DefinitionException, IOException {
  ...
  public XhtmlNode renderUnion(ProfileComparison comp, String id, String prefix, String corePath) throws FHIRException, IOException {    
    StructureDefinitionRenderer sdr = new StructureDefinitionRenderer(new RenderingContext(utilsLeft.getContext(), new RendererFactory(), null, utilsLeft.getTerminologyServiceOptions(), corePath, prefix, utilsLeft.getContext().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER).setPkp(this));
    return sdr.generateTable(new Renderer.RenderingStatus(), corePath, comp.union, false, prefix, false, id, true, corePath, prefix, false, true, null, false, sdr.getContext().withUniqueLocalPrefix("u"), "u", null, "C1");
  }
  ...
  public XhtmlNode renderIntersection(ProfileComparison comp, String id, String prefix, String corePath) throws FHIRException, IOException {
    StructureDefinitionRenderer sdr = new StructureDefinitionRenderer(new RenderingContext(utilsLeft.getContext(), new RendererFactory(), null, utilsLeft.getTerminologyServiceOptions(), corePath, prefix, utilsLeft.getContext().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER).setPkp(this));
    return sdr.generateTable(new Renderer.RenderingStatus(), corePath, comp.intersection, false, prefix, false, id, true, corePath, prefix, false, true, null, false, sdr.getContext().withUniqueLocalPrefix("i"), "i", null, "C2");
  }
    ...
    row.setAnchor(path);
      ...
      if (tail(path).equals("extension")) {
        if (elementIsComplex(combined))
          row.setIcon("icon_extension_complex.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_COMPLEX));
        else
          row.setIcon("icon_extension_simple.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE));
        ext = true;
      } else if (tail(path).equals("modifierExtension")) {
        if (elementIsComplex(combined))
          row.setIcon("icon_modifier_extension_complex.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_COMPLEX));
        else
          row.setIcon("icon_modifier_extension_simple.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_EXTENSION_SIMPLE));
      } else if (hasChoice(combined)) {
        if (allAreReference(combined))
          row.setIcon("icon_reference.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_REFERENCE));
        else {
          row.setIcon("icon_choice.gif", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_CHOICE));
          typesRow = row;
        }
      } else if (combined.either().getDef().hasContentReference())
        row.setIcon("icon_reuse.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_REUSE));
      else if (isPrimitive(combined))
        row.setIcon("icon_primitive.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_PRIMITIVE));
      else if (hasTarget(combined))
        row.setIcon("icon_reference.png", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_REFERENCE));
      else if (isDatatype(combined))
        row.setIcon("icon_datatype.gif", session.getI18n().formatPhrase(RenderingI18nContext.TEXT_ICON_DATATYPE));
      else
        row.setIcon("icon_resource.png", session.getI18n().formatPhrase(RenderingI18nContext.GENERAL_RESOURCE));
      ...
      StructureDefinitionRenderer sdrLeft = new StructureDefinitionRenderer(new RenderingContext(utilsLeft.getContext(), new RendererFactory(), null, utilsLeft.getTerminologyServiceOptions(), corePath, prefix, utilsLeft.getContext().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER).setPkp(this));
      StructureDefinitionRenderer sdrRight= new StructureDefinitionRenderer(new RenderingContext(utilsRight.getContext(), new RendererFactory(), null, utilsRight.getTerminologyServiceOptions(), corePath, prefix, utilsRight.getContext().getLocale(), RenderingContext.ResourceRendererMode.TECHNICAL, RenderingContext.GenerationRules.IG_PUBLISHER).setPkp(this));
      ...
      if (combined.hasLeft()) {
        frame(sdrLeft.genElementCells(new Renderer.RenderingStatus(), gen, combined.getLeft().getDef(),  "??", true, corePath, prefix, root, false, false, combined.getLeft().getSrc(), typesRow, row, true, ext, used , ref, nc, false, false, sdrLeft.getContext(), children.size() > 0, defPath, anchorPrefix, new ArrayList<ElementDefinition>(), null, false), leftColor);
      } else {
        frame(spacers(row, 4, gen), leftColor);
      }
      if (combined.hasRight()) {
        frame(sdrRight.genElementCells(new Renderer.RenderingStatus(), gen, combined.getRight().getDef(), "??", true, corePath, prefix, root, false, false, combined.getRight().getSrc(), typesRow, row, true, ext, used, ref, nc, false, false, sdrRight.getContext(), children.size() > 0, defPath, anchorPrefix, new ArrayList<ElementDefinition>(), null, false), rightColor);
      } else {
...
@Override
public BindingResolution resolveBinding(StructureDefinition def, ElementDefinition.ElementDefinitionBindingComponent binding, String path)
