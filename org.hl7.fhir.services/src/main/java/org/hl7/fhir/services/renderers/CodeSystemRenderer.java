...
import org.hl7.fhir.services.renderers.utils.RenderingContext.MultiLanguagePolicy;
...
public class CodeSystemRenderer extends TerminologyRenderer {
  ...
  private void generateFilters(XhtmlNode x, CodeSystem cs) {
    if (cs.hasFilter()) {
      ...
      for (CodeSystemFilterComponent f : cs.getFilterList()) {
        ...
        XhtmlNode td = tr.td();
        for (Enumeration<org.hl7.fhir.model.core.Enumerations.FilterOperator> t : f.getOperatorList())
  ...
  private void listConceptLanguages(CodeSystem cs, ConceptDefinitionComponent c, List<String> langs) {
    for (ConceptDefinitionDesignationComponent cd : c.getDesignationList()) {
  ...
  private int countConcepts(List<ConceptDefinitionComponent> list) {
    ...
    for (ConceptDefinitionComponent c : list)
      if (c.hasConcept())
        count = count + countConcepts(c.getConceptList());
    return count;
  }
  ...
  private void addLanguageRow(ConceptDefinitionComponent c, XhtmlNode t, List<String> langs) {
    ...
    for (String lang : langs) {
      ...
      for (ConceptDefinitionDesignationComponent designation : c.getDesignationList()) {
