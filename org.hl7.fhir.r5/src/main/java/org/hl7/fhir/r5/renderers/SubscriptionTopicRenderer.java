  ...
  public void buildNarrative(RenderingStatus status, XhtmlNode x, ResourceWrapper st) throws FHIRFormatError, DefinitionException, IOException, FHIRException, EOperationOutcome {
    ...
    if (st.has("resourceTrigger")) {
      ...
      for (ResourceWrapper rt : st.children("resourceTrigger")) {
        ...
        for (ResourceWrapper t : rt.children("supportedInteraction")) {          
          tr.value(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_INT), t);
        }
        if (rt.has("queryCriteria")) {
          StringBuilder md = new StringBuilder(); 
          ResourceWrapper qc = rt.child("queryCriteria");
          if (qc.has("previous")) {
            md.append(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_PREV, qc.primitiveValue("previous")+"\r\n")+" ");
          }
          if (qc.has("resultForCreate")) {
            md.append(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_CREATE, qc.primitiveValue("resultForCreate")+"\r\n")+" ");
          }
          if (qc.has("current")) {
            md.append(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_CURR, qc.primitiveValue("current")+"\r\n")+" ");
          }
          if (qc.has("previous")) {
            md.append(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_DELETE, qc.primitiveValue("resultForDelete")+"\r\n")+" ");
          }
          if (qc.has("requireBoth")) {
            md.append(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_REQ, qc.primitiveValue("requireBoth")+"\r\n")+" ");
          }
          tr.value(context.formatPhrase(RenderingI18nContext.GENERAL_CRIT), wrapNC(new MarkdownType(md.toString())));          
        }
        if (rt.has("fhirPathCriteria")) {
          tr.value(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_FHIR_PATH), rt.child("fhirPathCriteria"));
        }
        if (rt.has("description")) {
          tr.value(context.formatPhrase(RenderingI18nContext.GENERAL_DESC), rt.child("description"));
        }
      }
    ...
    if (st.has("eventTrigger")) {
      ...
      for (ResourceWrapper rt : st.children("eventTrigger")) {
        ...
        if (rt.has("event(")) {
          tr.value(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_EVENT), rt.child("event"));
        }
        if (rt.has("description")) {
          tr.value(context.formatPhrase(RenderingI18nContext.GENERAL_DESC), rt.child("description"));
        }
    ...
    if (st.has("canFilterBy")) {
      ...
      for (ResourceWrapper rt : st.children("canFilterBy")) {
        ...
        if (rt.has("filterParameter")) {
          tr.value(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_FILT_PAR), rt.child("filterParameter"));
        }
        if (rt.has("filterDefinition")) {
          tr.value(context.formatPhrase(RenderingI18nContext.SUB_TOPIC_FILT_DEF), rt.child("filterDefinition"));
        }
        for (ResourceWrapper t : rt.children("comparator")) {
          tr.value(context.formatPhrase(RenderingI18nContext.GENERAL_COMPARATORS), t);
        }
