...
public class SQLOnFhirTests {
    ...
    public List<Base> fetch(String resourceType) {
      ...
      for (JsonObject res : details.resources) {
        try {
          ...
          Resource resource = new org.hl7.fhir.model.core.formats.JsonParser(TestingUtilities.getSharedWorkerContext()).parse(src);
  ...
  public static Stream<Arguments> data() throws ParserConfigurationException, SAXException, IOException {
    ...
    JsonArray testFiles = (JsonArray) JsonParser.parse(TestingUtilities.loadTestResourceStream("r6", "sql-on-fhir", "manifest.json"));

    for (String s : testFiles.asStrings()) {
      JsonObject json = JsonParser.parseObject(TestingUtilities.loadTestResourceStream("r6", "sql-on-fhir", s));
  ...
  @MethodSource("data")
  public void test(String name, TestDetails test) throws FileNotFoundException, IOException, FHIRException, FHIRException, UcumException {
    this.details = test;
    ...
    System.out.println("");
    System.out.println("----------------------------------------------------------------------------");
    System.out.println(test.name+" ("+runner.getContext().getFHIRVersion()+")");
