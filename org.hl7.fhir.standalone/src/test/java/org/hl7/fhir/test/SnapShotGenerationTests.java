...
public class SnapShotGenerationTests {
  ...
  public class TestPKP implements ProfileKnowledgeProvider {
    ...
    public String getLinkForProfile(StructureDefinition profile, String url) {
      StructureDefinition sd = testContext.fetchResource(StructureDefinition.class, url, VersionResolutionRules.defaultRule());
      if (sd == null)
        return url + "|" + url;
      else
  ...
  private static class SnapShotGenerationTestsContext implements IHostApplicationServices {
    ...
    public Resource fetchFixture(String id) {
      ...
      if (id.equals("patient"))
        return testContext.fetchResource(StructureDefinition.class, "http://hl7.org/fhir/StructureDefinition/Patient", VersionResolutionRules.defaultRule());
      if (id.equals("valueset"))
        return testContext.fetchResource(StructureDefinition.class, "http://hl7.org/fhir/StructureDefinition/ValueSet", VersionResolutionRules.defaultRule());
      if (id.equals("organization"))
        return testContext.fetchResource(StructureDefinition.class, "http://hl7.org/fhir/StructureDefinition/Organization", VersionResolutionRules.defaultRule());
      if (id.equals("operationoutcome"))
        return testContext.fetchResource(StructureDefinition.class, "http://hl7.org/fhir/StructureDefinition/OperationOutcome", VersionResolutionRules.defaultRule());
      if (id.equals("parameters"))
        return testContext.fetchResource(StructureDefinition.class, "http://hl7.org/fhir/StructureDefinition/Parameters", VersionResolutionRules.defaultRule());
...
//      }
  ...
  public static Stream<Arguments> data() throws ParserConfigurationException, IOException, FHIRFormatError, SAXException {
    ...
    Document tests = XMLUtil.parseToDom(TestingUtilities.loadTestResource("r6", "snapshot-generation", "manifest.xml"));
  ...
  private void testSort(TestDetails test, SnapShotGenerationTestsContext context) throws DefinitionException, FHIRException, IOException {
    ...
    test.setOutput(test.getSource().copy(Base.COPY_DATA));
...
//    IOUtils.copy(TestingUtilities.loadTestResourceStream("r6", "snapshot-generation", test.getId() + "-expected.xml"), ManagedFileAccess.outStream(TestingUtilities.tempFile("snapshot", test.getId() + "-expected.xml")));
  ...
  private void testGen(boolean fail, TestDetails test, SnapShotGenerationTestsContext context) throws Exception {
    ...
    if (!testContext.hasPackage(CommonPackages.ID_XVER, CommonPackages.VER_XVER)) {
      ...
      testContext.getManager().loadFromPackage(npm, new TestPackageLoader(Utilities.stringSet("StructureDefinition"),testContext));
    }
    pu.setXver(XVerExtensionManagerFactory.createExtensionManager(testContext));
    if (test.isSort()) {
      List<String> errors = new ArrayList<String>();
      int lastCount = output.getDifferential().getElementList().size();
...
//      IOUtils.copy(TestingUtilities.loadTestResourceStream("r6", "snapshot-generation", test.getId() + "-expected.xml"), ManagedFileAccess.outStream(dst));
      ...
      t1.setText(null);
      StructureDefinition t2 = test.output.copy(Base.COPY_DATA);
      t2.setText(null);
...
//        DiffUtils.testDiff(dst.getAbsolutePath(), actualFilePath);
  ...
  private StructureDefinition getSD(String url, SnapShotGenerationTestsContext context) throws DefinitionException, FHIRException, IOException {
    ...
    if (sd == null) {
      sd = testContext.fetchResource(StructureDefinition.class, url, VersionResolutionRules.defaultRule());
    } 
    if (sd == null) {
      if (url.contains("|")) {
