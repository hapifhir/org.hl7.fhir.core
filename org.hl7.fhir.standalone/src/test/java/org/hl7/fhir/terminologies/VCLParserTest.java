...
public class VCLParserTest {
  ...
  public void testSimpleDisjunction() throws VCLParser.VCLParseException {
    ...
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(2, include.getConceptList().size());
    assertEquals("subscriber", include.getConceptList().get(0).getCode());
    assertEquals("provider", include.getConceptList().get(1).getCode());
  }
  ...
  public void testMultipleSystemsWithCodes() throws VCLParser.VCLParseException {
    ...
    assertEquals(3, vs.getCompose().getIncludeList().size());

    // Check LOINC system
    ConceptSetComponent loincInclude = vs.getCompose().getIncludeList().get(0);
    assertEquals("http://loinc.org", loincInclude.getSystem());
    assertEquals(9, loincInclude.getConceptList().size());
    assertEquals("41995-2", loincInclude.getConceptList().get(0).getCode());

    // Check SNOMED system
    ConceptSetComponent snomedInclude = vs.getCompose().getIncludeList().get(1);
    assertEquals("http://snomed.info/sct", snomedInclude.getSystem());
    assertEquals(6, snomedInclude.getConceptList().size());

    // Check CPT system
    ConceptSetComponent cptInclude = vs.getCompose().getIncludeList().get(2);
    assertEquals("http://www.ama-assn.org/go/cpt", cptInclude.getSystem());
    assertEquals(4, cptInclude.getConceptList().size());
  }
  ...
  public void testComplexExpressionWithExclusion() throws VCLParser.VCLParseException {
    ...
    // Check exclusion
    ConceptSetComponent exclude = vs.getCompose().getExcludeList().get(0);
    assertEquals("http://loinc.org", exclude.getSystem());
    assertEquals(1, exclude.getConceptList().size());
    assertEquals("76573-5", exclude.getConceptList().get(0).getCode());
  }
  ...
  public void testDifferentSystemsDisjunction() throws VCLParser.VCLParseException {
    ...
    assertEquals(2, vs.getCompose().getIncludeList().size());

    ConceptSetComponent include1 = vs.getCompose().getIncludeList().get(0);
    assertEquals("http://hl7.org/fhir/paymentstatus", include1.getSystem());
    assertEquals("paid", include1.getConceptList().get(0).getCode());

    ConceptSetComponent include2 = vs.getCompose().getIncludeList().get(1);
    assertEquals("http://hl7.org/fhir/payeetype", include2.getSystem());
    assertEquals("provider", include2.getConceptList().get(0).getCode());
  }
  ...
  public void testIncludeValueSet() throws VCLParser.VCLParseException {
    ...
    assertEquals(1, vs.getCompose().getIncludeList().size());
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(1, include.getValueSetList().size());
    assertEquals("http://hl7.org/fhir/ValueSet/payeetype", include.getValueSetList().get(0).primitiveValue());
  }
  ...
  public void testFilterWithoutSystem() throws VCLParser.VCLParseException {
    ...
    assertEquals(1, vs.getCompose().getIncludeList().size());
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(1, include.getFilterList().size());

    ConceptSetFilterComponent filter = include.getFilterList().get(0);
  ...
  public void testSimpleCodesInParentheses() throws VCLParser.VCLParseException {
    ...
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(2, include.getConceptList().size());
    assertEquals("10007-3", include.getConceptList().get(0).getCode());
    assertEquals("10008-1", include.getConceptList().get(1).getCode());
  }
  ...
  public void testSingleCode() throws VCLParser.VCLParseException {
    ...
    assertEquals(1, vs.getCompose().getIncludeList().size());
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(1, include.getConceptList().size());
    assertEquals("A", include.getConceptList().get(0).getCode());
  }
  ...
  public void testConjunctionWithComma() throws VCLParser.VCLParseException {
    ...
    assertEquals(2, include.getConceptList().size());
    assertEquals("A", include.getConceptList().get(0).getCode());
    assertEquals("B", include.getConceptList().get(1).getCode());
  }
  ...
  public void testDisjunctionWithSemicolon() throws VCLParser.VCLParseException {
    ...
    assertEquals(2, vs.getCompose().getIncludeList().size());
    assertEquals("A", vs.getCompose().getIncludeList().get(0).getConceptList().get(0).getCode());
    assertEquals("B", vs.getCompose().getIncludeList().get(1).getConceptList().get(0).getCode());
  }
  ...
  public void testMultipleStatusCodes() throws VCLParser.VCLParseException {
    ...
    assertEquals(1, vs.getCompose().getIncludeList().size());
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(4, include.getConceptList().size());
    assertEquals("in-progress", include.getConceptList().get(0).getCode());
    assertEquals("aborted", include.getConceptList().get(1).getCode());
    assertEquals("completed", include.getConceptList().get(2).getCode());
    assertEquals("entered-in-error", include.getConceptList().get(3).getCode());
  }
  ...
  public void testIncludeExcludeSameValueSet() throws VCLParser.VCLParseException, IOException {
    ...
    assertEquals("http://csiro.au/fhir/ValueSet/selfexclude",
      vs.getCompose().getIncludeList().get(0).getValueSetList().get(0).primitiveValue());
    assertEquals("http://csiro.au/fhir/ValueSet/selfexclude",
      vs.getCompose().getExcludeList().get(0).getValueSetList().get(0).primitiveValue());
  }
...
//  }
  ...
  public void testMultipleFiltersWithCommas() throws VCLParser.VCLParseException {
    ...
    assertEquals(1, vs.getCompose().getIncludeList().size());
    ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
    assertEquals(2, include.getFilterList().size()); // Should have 2 filters

    // Check the filters
    assertEquals("has_ingredient", include.getFilterList().get(0).getProperty());
    assertEquals("1886", include.getFilterList().get(0).getValue());
    assertEquals("has_dose_form", include.getFilterList().get(1).getProperty());
    assertEquals("317541", include.getFilterList().get(1).getValue());
  }
  ...
  public void testSimpleCodes(String code) throws VCLParser.VCLParseException {
    ...
    if (!"*".equals(code)) {
      ConceptSetComponent include = vs.getCompose().getIncludeList().get(0);
      if (include.getConceptList().size() > 0) {
        String expectedCode = code.startsWith("\"") && code.endsWith("\"")
          ? code.substring(1, code.length() - 1)
          : code;
        assertEquals(expectedCode, include.getConceptList().get(0).getCode());
      }
    }
  }
  ...
  public void testValueSetIncludes(String vcl) throws VCLParser.VCLParseException {
    ...
    assertNotNull(vs);
    assertEquals(1, vs.getCompose().getIncludeList().size());
    assertEquals(1, vs.getCompose().getIncludeList().get(0).getValueSetList().size());
  }
  ...
  private void printValueSetStructure(ValueSet vs) {
    ...
    System.out.println("  Includes: " + vs.getCompose().getIncludeList().size());
    for (int i = 0; i < vs.getCompose().getIncludeList().size(); i++) {
      ConceptSetComponent include = vs.getCompose().getIncludeList().get(i);
      System.out.printf("    Include %d: system=%s, concepts=%d, filters=%d, valueSets=%d%n",
        i, include.getSystem(), include.getConceptList().size(),
        include.getFilterList().size(), include.getValueSetList().size());
    }
    System.out.println("  Excludes: " + vs.getCompose().getExcludeList().size());
    for (int i = 0; i < vs.getCompose().getExcludeList().size(); i++) {
      ConceptSetComponent exclude = vs.getCompose().getExcludeList().get(i);
      System.out.printf("    Exclude %d: system=%s, concepts=%d, filters=%d, valueSets=%d%n",
        i, exclude.getSystem(), exclude.getConceptList().size(),
        exclude.getFilterList().size(), exclude.getValueSetList().size());
    }
  }
}