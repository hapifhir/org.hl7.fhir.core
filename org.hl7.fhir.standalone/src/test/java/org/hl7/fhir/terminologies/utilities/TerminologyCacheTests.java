...
import org.hl7.fhir.context.CacheTestUtils;
import org.hl7.fhir.model.Base;
import org.hl7.fhir.model.utilities.formats.IParser;
import org.hl7.fhir.model.core.*;
import org.hl7.fhir.model.utilities.formats.OutputStyle;
import org.hl7.fhir.services.terminology.CacheToken;
...
public class TerminologyCacheTests implements ResourceLoaderTests {
  ...
  private TerminologyCache createTerminologyCache() throws IOException {
    ...
    TerminologyCache terminologyCache = new TerminologyCache(lock, null, TestingUtilities.getSharedWorkerContext());
  ...
  class PersistenceTests {
    ...
    final CacheToken codeableConceptTokenA;

    final CacheToken expansionTokenA;
    ...
    PersistenceTests() throws IOException {
      ...
      terminologyCapabilities.getExpansion().setParameterList(Arrays.asList());
      ...
      terminologyCacheA = new TerminologyCache(lock, tempCacheDirectory.toString(), TestingUtilities.getSharedWorkerContext());
    ...
    public void testCachePersistence() throws IOException {
      ...
      //Create another cache using the same directory, and check that it gives the same results.
        ...
        TerminologyCache terminologyCacheB = new TerminologyCache(lock, tempCacheDirectory.toString(), TestingUtilities.getSharedWorkerContext());
    ...
    public void testCacheExpiresCapabilitiesFiles() throws IOException, InterruptedException {
      ...
      Thread.sleep(200L);
      TerminologyCache terminologyCacheB = new TerminologyCache(lock, tempCacheDirectory.toString(), 100L, TestingUtilities.getSharedWorkerContext());
  ...
  public void testCacheMakesCopiesOfResults() throws IOException{
    ...
    TerminologyCache terminologyCache = new TerminologyCache(lock, tempCacheDirectory.toString(), TestingUtilities.getSharedWorkerContext());
    ...
    CacheToken codingToken = terminologyCache.generateValidationToken(CacheTestUtils.validationOptions,
      coding, valueSet, new Parameters());
    ...
    CacheToken codeableConceptToken = terminologyCache.generateValidationToken(CacheTestUtils.validationOptions,
      concept, valueSet, new Parameters());
  ...
  private static Stream<Arguments> getExpansionTokenParams() {
    ...
    ValueSet withInclude = baseValueSet.copy(Base.COPY_DATA);
    withInclude.getCompose().setIncludeList(Arrays.asList(include));

    ValueSet withExclude = baseValueSet.copy(Base.COPY_DATA);
    withExclude.getCompose().setExcludeList(Arrays.asList(exclude));

    ValueSet withExpansion = baseValueSet.copy(Base.COPY_DATA);
    withExpansion.getExpansion().setContainsList(Arrays.asList(containsComponent));

    ValueSet allSystem = baseValueSet.copy(Base.COPY_DATA);
    allSystem.getCompose().setExcludeList(Arrays.asList(exclude));
  ...
  public void testExpansionTokenInclude(ValueSet valueSet, String expectedName) throws IOException, URISyntaxException {
    ...
    CacheToken expansionTokenHierarchical = terminologyCache.generateExpandToken(valueSet, new ExpansionOptions().withHierarchical(true));
  ...
  public void testExpansionToken() throws IOException, URISyntaxException {
    ...
    valueSet.getCompose().setIncludeList(Arrays.asList(include));
    valueSet.getCompose().setExcludeList(Arrays.asList(exclude));
    valueSet.getExpansion().setContainsList(Arrays.asList(containsComponent));
  ...
  public void testGetVSEssence() throws IOException {
    ...
    assertEquals(vs.getExpansion().getParameterList(), vse.getExpansion().getParameterList());
    assertEquals(vs.getExpansion().getContainsList(), vse.getExpansion().getContainsList());
  ...
  void testDistinctRequestsGetDistinctEntries() throws IOException {
    ...
    CacheToken token1 = cache.generateValidationToken(CacheTestUtils.validationOptions, coding1, valueSet, new Parameters());
    CacheToken token2 = cache.generateValidationToken(CacheTestUtils.validationOptions, coding2, valueSet, new Parameters());
  ...
  void testHitAndMissCountersTrackLookups() throws IOException {
    ...
    CacheToken token = cache.generateValidationToken(CacheTestUtils.validationOptions, coding, valueSet, new Parameters());
  ...
  void testSubsumesCacheRoundTrip() throws IOException {
    ...
    TerminologyCache cacheA = new TerminologyCache(lock, dir.toString(),TestingUtilities.getSharedWorkerContext());
    ...
    CacheToken token = cacheA.generateSubsumesToken(CacheTestUtils.validationOptions, parent, child, new Parameters());
    ...
    TerminologyCache cacheB = new TerminologyCache(lock, dir.toString(),TestingUtilities.getSharedWorkerContext());
  ...
  void testGetServerIdDisambiguatesCollidingAddresses() throws IOException {
    Object lock = new Object();
    Path dir = createTempCacheDirectory();
    TerminologyCache cache = new TerminologyCache(lock, dir.toString(),TestingUtilities.getSharedWorkerContext());
  ...
  void testLoadSkipsMalformedEntryAndLoadsTheRest() throws IOException {
    ...
    CacheToken token1 = cacheA.generateValidationToken(CacheTestUtils.validationOptions, coding1, valueSet, new Parameters());
    CacheToken token2 = cacheA.generateValidationToken(CacheTestUtils.validationOptions, coding2, valueSet, new Parameters());
  ...
  void testPersistentEntriesAreCappedFifo() throws IOException {
    ...
    try {
      TerminologyCache cache = createEmptyTerminologyCache();
      ValueSet valueSet = new ValueSet();
      CacheToken[] tokens = new CacheToken[5];
      for (int i = 0; i < 5; i++) {
  ...
  void testLoadTrimsOversizedCacheToLimit() throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < 5; i++) {
        Coding coding = new Coding().setSystem("http://example.org/sys").setCode("code" + i);
        CacheToken token = cacheA.generateValidationToken(CacheTestUtils.validationOptions, coding, valueSet, new Parameters());
  ...
  void testGetExpansionAfterUnloadThrows() throws IOException {
    ...
    CacheToken token = cache.generateExpandToken(valueSet, new ExpansionOptions().withHierarchical(true));
    cache.unload();
  ...
  void testGetSubsumesAfterUnloadThrows() throws IOException {
    TerminologyCache cache = createEmptyTerminologyCache();
    CacheToken token = cache.generateSubsumesToken(CacheTestUtils.validationOptions,
