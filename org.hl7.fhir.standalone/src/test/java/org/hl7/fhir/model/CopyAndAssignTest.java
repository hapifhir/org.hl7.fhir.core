 ...
 * bugs were hiding.
 * <p>
 * This is the R6 twin of org.hl7.fhir.r5.model.CopyAndAssignTest - keep the two in step.
 */
public class CopyAndAssignTest {

  private static final EnumSet<CopyObjectOptions> OPTIONS = Base.COPY_ALL;
  ...
  public static Stream<Arguments> composites() {
    ...
    res.add(Arguments.of("Patient.ContactComponent", patient().getContactList().get(0)));
    return res.stream();
  }
  ...
  public void copyValuesCarriesThePrimitiveValue() {
    ...
    src.copyValues(dst, OPTIONS);
    assertEquals("hello", dst.getValue());
  }

  @Test
  public void base64BinaryCopyKeepsIdAndExtensions() {
    // Base64BinaryType.copy() used to be "return new Base64BinaryType(modelContext, getValue())" -
    // no copyValues call at all, so the id and the extensions were dropped
    Base64BinaryType src = new Base64BinaryType("aGVsbG8=");
    src.setId("b64-1");
    src.addExtension("http://example.org/ext", new StringType("x"));
    Base64BinaryType dst = src.copy(OPTIONS);
  ...
  public void dateTimeKeepsPrecisionAndTimezone() {
    ...
    assertEquals("2024-03", new DateTimeType("2024-03").copy(OPTIONS).getValueAsString());
  }
  ...
  public void dateTimeDistinguishesZuluFromAZeroOffset() {
    ...
    assertTrue(zulu.copy(OPTIONS).isTimeZoneZulu());
    assertFalse(zero.copy(OPTIONS).isTimeZoneZulu());
    assertEquals("2024-03-05T10:11:12Z", zulu.copy(OPTIONS).getValueAsString());
    assertEquals("2024-03-05T10:11:12+00:00", zero.copy(OPTIONS).getValueAsString());
  }
  ...
  public void partialDatesKeepTheirPrecision(String value) {
    ...
    assertEquals(value, src.copy(OPTIONS).getValueAsString());
  }
