...
import org.hl7.fhir.standalone.terminology.expansion.OperationIsTooCostly;
...
public class ValueSetProcessBase {
  ...
  public static class AlternateCodesProcessingRules {
    ...
    public void seeParameters(Parameters pp) {
      for (ParametersParameterComponent p : pp.getParameterList()) {
  ...
  protected List<OperationOutcomeIssueComponent> makeIssue(IssueSeverity level, IssueType type, String location, String message, OpIssueCode code, String server, String msgId) {
    ...
    switch (level) {
    case ERROR:
      result.setSeverity(org.hl7.fhir.model.core.OperationOutcome.IssueSeverity.ERROR);
      break;
    case FATAL:
      result.setSeverity(org.hl7.fhir.model.core.OperationOutcome.IssueSeverity.FATAL);
      break;
    case INFORMATION:
      result.setSeverity(org.hl7.fhir.model.core.OperationOutcome.IssueSeverity.INFORMATION);
      break;
    case WARNING:
      result.setSeverity(org.hl7.fhir.model.core.OperationOutcome.IssueSeverity.WARNING);
      break;
    }
    result.setCode(type);
    if (location != null) {
  ...
  protected UnknownValueSetException failWithUnknownVSException(String msgId, boolean check, Object... params) {
    String msg = context.formatMessage(msgId, params);
    allErrors.add(msg);
