...
public class TerminologyOperationContext {
  ...
  public static class TerminologyServiceProtectionException extends FHIRException {
    ...
    public TerminologyServiceProtectionException(String message, TerminologyServiceErrorClass error, IssueType type, String msgId, OpIssueCode code) {
      super(message);
      this.error = error;
      this.type = type;
      this.msgId = msgId;
    ...
    public TerminologyServiceProtectionException(String message, TerminologyServiceErrorClass error, IssueType type, String msgId, OpIssueCode code, String diagnostics) {
      super(message);
      this.error = error;
      this.type = type;
      this.msgId = msgId;
  ...
  public void deadCheck(String note) {
    ...
    if (deadTime != 0 &&  System.currentTimeMillis() > deadTime) {
     ...
     throw new TerminologyServiceProtectionException(worker.formatMessage(I18nConstants.VALUESET_TOO_COSTLY_TIME, contexts.get(0), EXPANSION_DEAD_TIME_SECS, name+" (local)"), TerminologyServiceErrorClass.TOO_COSTLY, IssueType.TOOCOSTLY, I18nConstants.VALUESET_TOO_COSTLY_TIME, OpIssueCode.VSProcessing);
    }
  }
  
  public void seeContext(String context) {
    if (contexts.contains(context)) {
      throw new TerminologyServiceProtectionException(worker.formatMessage(I18nConstants.VALUESET_CIRCULAR_REFERENCE, context, contexts.toString()), TerminologyServiceErrorClass.PROCESSING, IssueType.PROCESSING, I18nConstants.VALUESET_CIRCULAR_REFERENCE, OpIssueCode.VSProcessing);
    }
