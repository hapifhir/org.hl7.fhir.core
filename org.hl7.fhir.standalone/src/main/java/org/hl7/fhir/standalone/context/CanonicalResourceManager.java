...
public class CanonicalResourceManager<T extends CanonicalResource> {
  ...
  public boolean register(CanonicalResourceProxy canonicalResourceProxy, PackageInformation packageInfo) {
    ...
    CachedCanonicalResource<T> cachedCanonicalResource = new CachedCanonicalResource<T>(canonicalResourceProxy, packageInfo);
  ...
  public void see(T r, PackageInformation packgeInfo) {
    if (r != null) {
      ...
      if (packgeInfo != null) {
        CachedCanonicalResource<T> cr = indexedResources.get(packgeInfo.getVID()+":"+ r.getUrl() + "|" + r.getVersion());
        ...
        CachedCanonicalResource<T> cr = indexedResources.get(r.getUrl() + "|" + r.getVersion());
...
//    }
  ...
  private void addToSupplements(CachedCanonicalResource<T> cr) {
    ...
    if (surl != null) {
      List<CachedCanonicalResource<T>> list = supplements.get(surl);
      if (list == null) {
        list = new ArrayList<>();
  ...
  public List<T> getForUrl(String url) {
    ...
    List<CachedCanonicalResource<T>> list = listForUrl.get(url);
    if (list != null) {
  ...
  public List<T> getSupplements(String url, String version, List<String> pvlist) {
    ...
    List<T> res = new ArrayList<>();
    if (version != null) {
      List<CachedCanonicalResource<T>> list = supplements.get(url+"|"+version);
      if (list != null) {
        for (CachedCanonicalResource<T> t : list) {
    ...
    List<CachedCanonicalResource<T>> list = supplements.get(url);
    if (list != null) {
      for (CachedCanonicalResource<T> t : list) {
        possibleMatches = true;
