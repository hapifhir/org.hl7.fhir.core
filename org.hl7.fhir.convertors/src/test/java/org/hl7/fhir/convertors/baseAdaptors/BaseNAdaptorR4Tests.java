...
/**
 * Tests the metadata based (property) access part of the Base API on the R4 adaptor - both what is
 * read through the adaptor, and that what is written through it lands on the underlying R4 object.
 *
 ...
 *
 * Also tests user data, which the adaptor keeps on the wrapped R4 objects
 */
class BaseNAdaptorR4Tests {

  private static Patient loadPatient() throws IOException {
    return (Patient) new XmlParser().parse(TestingUtilities.loadTestResourceBytes("r4", "patient-example.xml"));
  }

  private static Observation loadObservation() throws IOException {
    return (Observation) new XmlParser().parse(TestingUtilities.loadTestResourceBytes("r4", "observation-example.xml"));
  }

  private static Base wrap(org.hl7.fhir.r4.model.Base r4) {
    return new BaseNAdaptorR4(ModelContext.fullCoreContext(), r4);
  }
  ...
  private static List<String> r4Names(List<org.hl7.fhir.r4.model.Property> properties) {
    List<String> result = new ArrayList<>();
    for (org.hl7.fhir.r4.model.Property p : properties) {
      result.add(p.getName());
    }
    return result;
  }
  ...
  void testPatient() throws IOException {
    ...
    Assertions.assertTrue(VersionUtilities.isR4Ver(base.getFHIRVersion()));
    ...
    Assertions.assertTrue(VersionUtilities.isR4Ver(address.getFHIRVersion()));
  ...
  void testChildrenMetadata() throws IOException {
    ...
    // all the defined children, whether they have values or not, in the R4 order
    Assertions.assertEquals(r4Names(obs.children()), names(children));
    Assertions.assertThrows(UnsupportedOperationException.class, () -> children.add(null));

    for (Property p : children) {
      org.hl7.fhir.r4.model.Property r4p = obs.getChildByName(p.getName());
      Assertions.assertNotNull(r4p, p.getName());
      Assertions.assertEquals(r4p.getTypeCode(), p.getTypeCode(), p.getName());
      Assertions.assertEquals(r4p.getDefinition(), p.getDefinition(), p.getName());
      Assertions.assertEquals(r4p.getMinCardinality(), p.getMinCardinality(), p.getName());
      Assertions.assertEquals(r4p.getMaxCardinality(), p.getMaxCardinality(), p.getName());
      Assertions.assertEquals(r4p.hasValues(), p.hasValues(), p.getName());
      Assertions.assertEquals(r4p.getValues().size(), p.getValues().size(), p.getName());
      for (int i = 0; i < p.getValues().size(); i++) {
        Base v = p.getValues().get(i);
        Assertions.assertTrue(VersionUtilities.isR4Ver(v.getFHIRVersion()), p.getName());
        Assertions.assertEquals(r4p.getValues().get(i).fhirType(), v.fhirType(), p.getName());
      }
    }
    ...
    Assertions.assertEquals(r4Names(obs.getValueQuantity().children()), names(quantity.getChildren()));
  }
  ...
  void testChoiceTypeRead() throws IOException {
    ...
    // (not "valueAge": R4's Extension.getNamedProperty only knows the type specific names for primitive types)
  ...
  void testChoiceTypeWrite() throws IOException {
    ...
    Assertions.assertTrue(obs.getValue() instanceof org.hl7.fhir.r4.model.StringType);
    ...
    Assertions.assertTrue(obs.getValue() instanceof org.hl7.fhir.r4.model.Quantity);
    ...
    // writes to the child land on the R4 object
    ...
    Assertions.assertTrue(obs.getEffective() instanceof org.hl7.fhir.r4.model.Period);
  ...
  void testPropertyWrite() throws IOException {
    ...
    Assertions.assertEquals(Observation.ObservationStatus.AMENDED, obs.getStatus());
    ...
    org.hl7.fhir.r4.model.Reference oldSubject = obs.getSubject();
    ...
    org.hl7.fhir.r4.model.CodeableConcept first = obs.getCategory().get(0);
  ...
  void testNewInstance() {
    Base base = new BaseNAdaptorR4(ModelContext.fullCoreContext(), "Observation");
    Assertions.assertEquals("Observation", base.fhirType());
    Assertions.assertTrue(VersionUtilities.isR4Ver(base.getFHIRVersion()));
    Assertions.assertTrue(base.isResource());
    Assertions.assertTrue(base.isEmpty());
    Assertions.assertEquals(r4Names(new Observation().children()), names(base.getChildren()));
    for (Property p : base.getChildren()) {
    ...
    Base type = new BaseNAdaptorR4(ModelContext.fullCoreContext(), "Quantity");
  ...
  void testUserData() throws IOException {
    ...
    // set on the R4 object: visible through the adaptor
    pat.setUserData("r4", "from-r4");
    Assertions.assertTrue(base.hasUserData("r4"));
    Assertions.assertEquals("from-r4", base.getUserString("r4"));

    Assertions.assertEquals(java.util.Set.of("s", "i", "o", "r4"), base.getUserDataNames());
    ...
    Assertions.assertNull(pat.getUserData("r4"));
    base.clearUserData();
  ...
  void testUserDataOnChildren() throws IOException {
    ...
    // the adaptor doesn't cache wrappers, but user data lives on the R4 object, so it survives re-wrapping
    Base code = base.getSingleChildValue("code", true);
    ...
    Base created = new BaseNAdaptorR4(ModelContext.fullCoreContext(), "Observation");
  ...
  void testUserDataValueTypes() throws IOException {
    ...
    Assertions.assertSame(r6, obs.getUserData("r6"), "R4 code sees the R6 object");
    org.hl7.fhir.r4.model.StringType r4 = new org.hl7.fhir.r4.model.StringType("r4");
    obs.setUserData("r4", r4);
    Assertions.assertSame(r4, base.getUserData("r4"), "R6 code sees the R4 object, not an adaptor");
    Assertions.assertFalse(base.getUserData("r4") instanceof Base);
    Base code = base.getSingleChildValue("code", true);
  ...
  void testCopyAndUserData() throws IOException {
    ...
    Assertions.assertTrue(VersionUtilities.isR4Ver(copy.getFHIRVersion()));
    ...
    Assertions.assertEquals(Observation.ObservationStatus.FINAL, obs.getStatus());
