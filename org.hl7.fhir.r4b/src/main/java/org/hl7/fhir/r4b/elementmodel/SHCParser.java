...
/**
 * this class is actually a smart health cards validator. It's going to parse
 * the JWT and assume that it contains a smart health card, which has a nested
 * bundle in it, and then validate the bundle.
 * 
 * See
 * https://spec.smarthealth.cards/#health-cards-are-encoded-as-compact-serialization-json-web-signatures-jws
 * 
 * This parser dose the JWT work, and then passes the JsonObject through to the
 * underlying JsonParser
 *
...
public class SHCParser extends ParserBase {
  ...
  public List<NamedElement> parse(InputStream stream)
      ...
      if (checkProperty(json, "$", "verifiableCredential", true, "Array")) {
        ...
        for (JsonElement e : arr) {
          if (!(e instanceof JsonPrimitive)) {
            logError(line(e), col(e), "$.verifiableCredential[" + i + "]", IssueType.STRUCTURE,
                "Wrong Property verifiableCredential in JSON Payload. Expected : String but found "
                    + JsonUtilities.type(e),
                IssueSeverity.ERROR);
          } else {
    ...
    for (String ssrc : list) {
      ...
      logError(1, 1, prefix + "JWT", IssueType.INFORMATIONAL, "The FHIR Validator does not check the JWT signature "
          + "(see https://demo-portals.smarthealth.cards/VerifierPortal.html or https://github.com/smart-on-fhir/health-cards-dev-tools) (Issuer = '"
          + jwt.getPayload().get("iss").getAsString() + "')", IssueSeverity.INFORMATION);
      ...
      for (JsonElement e : type) {
        if (!(e instanceof JsonPrimitive)) {
          logError(line(e), col(e), path + ".type[" + i + "]", IssueType.STRUCTURE,
              "Wrong Property Type in JSON Payload. Expected : String but found " + JsonUtilities.type(e),
              IssueSeverity.ERROR);
        } else {
      ...
      if (!types.contains("https://smarthealth.cards#health-card")) {
        logError(line(vc), col(vc), path, IssueType.STRUCTURE,
            "Card does not claim to be of type https://smarthealth.cards#health-card, cannot validate",
            IssueSeverity.ERROR);
        return res;
      }
      ...
      if (!VersionUtilities.versionMatches(context.getVersion(), fv.getAsString())) {
        logError(
            line(fv), col(fv), path + ".fhirVersion", IssueType.STRUCTURE, "Card claims to be of version "
                + fv.getAsString() + ", cannot be validated against version " + context.getVersion(),
            IssueSeverity.ERROR);
        return res;
      }
  ...
  public String getImpliedProfile() {
    if (types.contains("https://smarthealth.cards#covid19")
        && types.contains("https://smarthealth.cards#immunization")) {
  ...
  private boolean checkProperty(JsonObject obj, String path, String name, boolean required, String type) {
    ...
    if (e != null) {
      ...
      if (!type.equals(t)) {
        logError(line(e), col(e), path + "." + name, IssueType.STRUCTURE,
            "Wrong Property Type in JSON Payload. Expected : " + type + " but found " + t, IssueSeverity.ERROR);
      } else {
        return true;
      }
    } else if (required) {
      logError(line(obj), col(obj), path, IssueType.STRUCTURE, "Missing Property in JSON Payload: " + name,
          IssueSeverity.ERROR);
    } else {
      return true;
    }
  ...
  private void checkNamedProperties(JsonObject obj, String path, String... names) {
    for (Entry<String, JsonElement> e : obj.entrySet()) {
      if (!Utilities.existsInList(e.getKey(), names)) {
        logError(line(e.getValue()), col(e.getValue()), path + "." + e.getKey(), IssueType.STRUCTURE,
            "Unknown Property in JSON Payload", IssueSeverity.WARNING);
      }
    }
  }
  ...
  public void compose(Element e, OutputStream destination, OutputStyle style, String base)
    ...
    // because then we'd have to try to sign, and we're just not going to be doing
    // that from the element model
  }
  ...
  public JWT decodeJWT(String jwt) throws IOException, DataFormatException {
    ...
    if (jwt.length() > MAX_ALLOWED_SHC_LENGTH) {
      logError(-1, -1, "jwt", IssueType.TOOLONG, "JWT Payload limit length is " + MAX_ALLOWED_SHC_LENGTH
          + " bytes for a single image - this has " + jwt.length() + " bytes", IssueSeverity.ERROR);
    }
