...
public class CanonicalResourceManager<T extends CanonicalResource> {
  ...
  private class CachedCanonicalResource<T1 extends CanonicalResource> {
    ...
    public String toString() {
      return resource != null
          ? resource.fhirType() + "/" + resource.getId() + ": " + resource.getUrl() + "|" + resource.getVersion()
          : proxy.toString();
    }
  ...
  public void see(CachedCanonicalResource<T> cr) {
    ...
    if (cr.getPackageInfo() != null && cr.getPackageInfo().getId() != null
        && cr.getPackageInfo().getId().startsWith("hl7.terminology")
        && "http://nucc.org/provider-taxonomy".equals(cr.getUrl())) {
      return;
    }
    ...
    CachedCanonicalResource<T> existing = cr.hasVersion() ? map.get(cr.getUrl() + "|" + cr.getVersion())
        : map.get(cr.getUrl() + "|#0");
    ...
    if (rl.size() > 0) {
      ...
      if (version != null) {
        ...
        for (CachedCanonicalResource<T> t : rl) {
          if (VersionUtilities.isSemVer(t.getVersion(), true) && VersionUtilities.isSemVer(version, true) &&  VersionUtilities.versionMatches(t.getVersion(), version)) {
            latest = t;
          }
        }
