...
public class PublicationHacker {
  ...
  // publications. All of them will be or are fixed in a later version,
  // but fixing old versions is procedurally very difficult. Hence, these work
  // around fixes here
  ...
  public static StringType fixBindingDescriptions(IWorkerContext context, StringType s) {
    ...
    if (res.getValue().contains("LOINC is  (preferred)[http://build.fhir.org/terminologies.html#preferred]")) {
      res.setValue(res.getValue().replace("LOINC is  (preferred)[http://build.fhir.org/terminologies.html#preferred]",
          "LOINC is [preferred](" + Utilities.pathURL(context.getSpecUrl(), "terminologies.html#preferred)")));
    }
