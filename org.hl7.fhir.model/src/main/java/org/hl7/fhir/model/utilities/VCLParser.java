...
public class VCLParser {
  ...
  public VCLParser(IModelContext context) {
  ...
  private static class Parser {
    ...
    private ConceptSetComponent getCurrentConceptSet(boolean isExclusion) {
      if (isExclusion) {
        List<ConceptSetComponent> excludes = compose.getExcludeList();
        return excludes.isEmpty() ? createConceptSet(null, true) : excludes.get(excludes.size() - 1);
      } else {
        List<ConceptSetComponent> includes = compose.getIncludeList();
  ...
  public static ValueSet parseAndId(String vclExpression, IModelContext context) throws VCLParseException, IOException {
    ValueSet vs = parse(vclExpression);
    String json = new JsonParser(context).composeString(vs);
