...
public class LanguageUtils {
  ...
  private int importResourceFromTranslations(Base parent, Base element, List<TranslationUnit> translations, Set<TranslationUnit> usedUnits, String path) {
    int t = 0;
    if (element.isPrimitive() && isTranslatable(element, path) && element instanceof org.hl7.fhir.model.core.Element) {
      org.hl7.fhir.model.core.Element e = (org.hl7.fhir.model.core.Element) element;
      String base = element.primitiveValue();
      if (base != null) {
    ...
    for (Property c : element.getChildren()) {
  ...
  private int importFromTranslations(String path, Element parent, Element element, List<TranslationUnit> translations, Set<TranslationUnit> usedUnits) {
    ...
    List<Element> childrenCopy = List.copyOf(element.getChildList());
    ...
    for (TranslationUnit tu : list) {
      ...
      if (cdSrc == null) {
        ...
        if (subCode == null) {
          ...
          boolean found = false;
          for (ConceptDefinitionDesignationComponent d : cdSrc.getDesignationList()) {
          ...
          if (!found) {
            for (Extension e : cdSrc.getExtensionList()) {
  ...
  private void generateTranslations(List<TranslationUnit> list, StructureDefinition sd, String lang) {
    ...
    for (ContactDetail cd : sd.getContactList()) {
    ...
    for (StructureDefinitionMappingComponent map : sd.getMappingList()) {
    ...
    for (ElementDefinition ed : sd.getDifferential().getElementList()) {
      ...
      for (ElementDefinitionConstraintComponent con : ed.getConstraintList()) {
        addToList(list, lang, con, "StructureDefinition.element."+ed.getId()+"/constraint", "human", con.getHumanElement());
      }
      for (ElementDefinitionMappingComponent map : ed.getMappingList()) {
  ...
  private String getTranslation(Element e, String lang) {
    ...
    for (Element ext : e.getChildList()) {
      if ("Extension".equals(ext.fhirType()) && "http://hl7.org/fhir/StructureDefinition/translation".equals(ext.getNamedChildValue("url"))) {
        ...
        for (Element subExt : ext.getChildList()) {
  ...
  private static boolean scanForGeneratedNarrative(XhtmlNode x, String lang, RenderingI18nContext rc) {
    ...
    if (x.getContent() != null && x.getContent().contains( rc.formatPhrase(RenderingI18nContext.PROF_DRIV_GEN_NARR_TECH, "", "").trim())) {
      return true;
    }
  ...
  public boolean hasTranslation(org.hl7.fhir.model.core.Element e, String lang) {
    return getTranslation(e, lang) != null;
  }

  public String getTranslation(org.hl7.fhir.model.core.Element e, String lang) {
