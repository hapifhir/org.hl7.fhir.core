...
public class MatchetypeValidator {
  ...
  private boolean compareElements(List<ValidationMessage> messages, String path, Element expectedElement, Element actualElement) {
    ...
    for (Property en : actualElement.children()) {
      ...
      } else if (!patternMode && !isOptional(n, optionals)) {
        // $optional-properties$ means the property may differ either way: absent from the
        // resource when the matchetype has it, and present when the matchetype does not.
        // Only the first was honoured, so a server sending an optional property it was
        // never required to send - a Coding's version, say - failed as unexpected.
  ...
  private List<String> listOptionals(Element expectedElement) {
    ...
    if (expectedElement.hasExtension(EXT_OPT_PROPS)) {
      ...
      @SuppressWarnings("checkstyle:stringImplicitPatternUsage")
      //single literal character split
      String[] optProps = expectedElement.getExtensionString(EXT_OPT_PROP).split("\\,");
      for (String s : optProps) {
        res.add(s);
      }
    }
  ...
  private List<String> listCountOnlys(Element expectedElement) {
    ...
    if (expectedElement.hasExtension(EXT_OPT_COUNT)) {
      @SuppressWarnings("checkstyle:stringImplicitPatternUsage")
      //single literal character split
      String[] countProps = expectedElement.getExtensionString(EXT_OPT_COUNT).split("\\,");
      for (String s : countProps) {
        res.add(s);
      }
    }
    return res;
